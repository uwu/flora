...
impl UvLoopInner {
    ...
    pub(crate) fn has_alive_handles(&self) -> bool {
        for handle_ptr in self.timer_handles.borrow().values() {
            // SAFETY: Handle pointers in timer_handles are kept valid by the C caller until uv_close.
            let handle = unsafe { &**handle_ptr };
            if handle.flags & UV_HANDLE_ACTIVE != 0 && handle.flags & UV_HANDLE_REF != 0 {
                return true;
            }
        }
    ...
    fn stop_tty(&self, handle: *mut uv_tty_t) {
        ...
        unsafe {
            ...
            #[cfg(unix)]
            {
                ...
                #[cfg(target_os = "macos")]
                if let Some(tty::TtyReactor::SelectFallback(ref mut s)) = tty.internal_reactor {
...
pub fn uv_guess_handle(fd: c_int) -> uv_handle_type {
    ...
    #[cfg(unix)]
    {
        ...
        if unsafe { libc::getsockname(fd, &mut ss as *mut _ as *mut libc::sockaddr, &mut len) } != 0
        {
            return uv_handle_type::UV_UNKNOWN_HANDLE;
        }

        let mut sock_type: c_int = 0;
        let mut type_len: libc::socklen_t = std::mem::size_of::<c_int>() as libc::socklen_t;
        if unsafe {
            libc::getsockopt(
                fd,
                libc::SOL_SOCKET,
...
/// ### Safety
/// `loop_` must be a valid pointer to a `uv_loop_t` previously initialized by `uv_loop_init`.
pub unsafe fn uv_loop_get_inner_ptr(loop_: *const uv_loop_t) -> *const std::ffi::c_void {
    // SAFETY: Caller guarantees loop_ is valid and was initialized by uv_loop_init.
...
#[cfg_attr(feature = "uv_compat_export", unsafe(no_mangle))]
pub unsafe extern "C" fn uv_timer_init(loop_: *mut uv_loop_t, handle: *mut uv_timer_t) -> c_int {
...
/// `loop_` must be initialized by `uv_loop_init`. `handle` must be a valid, writable pointer.
#[cfg_attr(feature = "uv_compat_export", unsafe(no_mangle))]
pub unsafe extern "C" fn uv_idle_init(loop_: *mut uv_loop_t, handle: *mut uv_idle_t) -> c_int {
    // SAFETY: Caller guarantees both pointers are valid.
    unsafe {
...
#[cfg_attr(feature = "uv_compat_export", unsafe(no_mangle))]
pub unsafe extern "C" fn uv_idle_start(handle: *mut uv_idle_t, cb: uv_idle_cb) -> c_int {
...
#[cfg_attr(feature = "uv_compat_export", unsafe(no_mangle))]
pub unsafe extern "C" fn uv_prepare_start(handle: *mut uv_prepare_t, cb: uv_prepare_cb) -> c_int {
...
/// `loop_` must be initialized by `uv_loop_init`. `handle` must be a valid, writable pointer.
#[cfg_attr(feature = "uv_compat_export", unsafe(no_mangle))]
pub unsafe extern "C" fn uv_check_init(loop_: *mut uv_loop_t, handle: *mut uv_check_t) -> c_int {
    // SAFETY: Caller guarantees both pointers are valid.
    unsafe {
...
#[cfg_attr(feature = "uv_compat_export", unsafe(no_mangle))]
pub unsafe extern "C" fn uv_check_start(handle: *mut uv_check_t, cb: uv_check_cb) -> c_int {
...
#[cfg_attr(feature = "uv_compat_export", unsafe(no_mangle))]
pub unsafe extern "C" fn uv_close(handle: *mut uv_handle_t, close_cb: Option<uv_close_cb>) {
    // SAFETY: Caller guarantees handle is valid and initialized.
    unsafe {
