...
impl<T> RawArenaEntry<T> {
    #[inline(always)]
    unsafe fn next(entry: NonNull<RawArenaEntry<T>>) -> NonNull<RawArenaEntry<T>> {
    ...
    unsafe fn drop(entry: NonNull<RawArenaEntry<T>>) {
        unsafe {
            std::ptr::drop_in_place(std::ptr::addr_of_mut!((*entry.as_ptr()).value) as *mut T);
        }
    }
}
...
impl<T> RawArena<T> {
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        ...
        Self {
            ...
            signature: SIGNATURE,
            alloc,
            past_alloc_end: unsafe { NonNull::new_unchecked(alloc.as_ptr().add(capacity)) },
            max: alloc.into(),
            next: Cell::new(alloc),
    ...
    pub unsafe fn recycle(&self, data: NonNull<T>) -> bool {
        unsafe {
            ...
            if entry >= self.alloc && entry < self.past_alloc_end {
                ...
                std::alloc::dealloc(entry.as_ptr() as _, Layout::new::<RawArenaEntry<T>>());
            }
            emptied
        }
    }
...
impl<T> Drop for RawArena<T> {
    ...
    fn drop(&mut self) {
        ...
        unsafe { std::alloc::dealloc(self.alloc.as_ptr() as _, Self::layout(self.capacity)) }
    }
}
