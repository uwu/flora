...
pub type OpMetricsFactoryFn = Box<dyn Fn(OpId, usize, &OpDecl) -> Option<OpMetricsFn>>;
...
pub fn merge_op_metrics(
    ...
    Box::new(
        move |op, count, decl| match (fn1(op, count, decl), fn2(op, count, decl)) {
            (None, None) => None,
            (Some(a), None) => Some(a),
            (None, Some(b)) => Some(b),
...
pub fn dispatch_metrics_fast(opctx: &OpCtx, metrics: OpMetricsEvent) {
    ...
    unsafe { (opctx.metrics_fn.as_ref().unwrap_unchecked())(opctx, metrics, OpMetricsSource::Fast) }
}

#[doc(hidden)]
pub fn dispatch_metrics_slow(opctx: &OpCtx, metrics: OpMetricsEvent) {
    // SAFETY: this should only be called from ops where we know the function is Some
    unsafe { (opctx.metrics_fn.as_ref().unwrap_unchecked())(opctx, metrics, OpMetricsSource::Slow) }
}
...
pub fn dispatch_metrics_async(opctx: &OpCtx, metrics: OpMetricsEvent) {
    // SAFETY: this should only be called from ops where we know the function is Some
    unsafe {
        (opctx.metrics_fn.as_ref().unwrap_unchecked())(opctx, metrics, OpMetricsSource::Async)
    }
}
...
#[doc(hidden)]
pub unsafe extern "C" fn slow_metrics_dispatch(info: *const v8::FunctionCallbackInfo) {
    ...
    let opctx: &OpCtx = unsafe {
        &*(v8::Local::<v8::External>::cast_unchecked(parts.data).value() as *const OpCtx)
    };
