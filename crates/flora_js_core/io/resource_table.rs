...
impl ResourceTable {
    ...
    pub fn get<T: Resource>(&self, rid: ResourceId) -> Result<Rc<T>, ResourceError> {
        self.index
            .get(&rid)
    ...
    pub fn get_any(&self, rid: ResourceId) -> Result<Rc<dyn Resource>, ResourceError> {
        self.index
            .get(&rid)
            .cloned()
            .ok_or(ResourceError::BadResourceId)
    ...
    /// type `T` from `Rc<T>`.
    pub fn take<T: Resource>(&mut self, rid: ResourceId) -> Result<Rc<T>, ResourceError> {
    ...
    pub fn take_any(&mut self, rid: ResourceId) -> Result<Rc<dyn Resource>, ResourceError> {
    ...
    /// purposes within ops.
    pub fn get_fd(&self, rid: ResourceId) -> Result<ResourceHandleFd, ResourceError> {
    ...
    /// purposes within ops.
    pub fn get_socket(&self, rid: ResourceId) -> Result<ResourceHandleSocket, ResourceError> {
        let Some(handle) = self.get_any(rid)?.backing_handle() else {
            return Err(ResourceError::BadResourceId);
        };
    ...
    /// purposes within ops.
    pub fn get_handle(&self, rid: ResourceId) -> Result<ResourceHandle, ResourceError> {
        let Some(handle) = self.get_any(rid)?.backing_handle() else {
            return Err(ResourceError::BadResourceId);
        };
        if !handle.is_valid() {
