...
impl BufView {
    ...
    pub fn split_off(&mut self, at: usize) -> Self {
        ...
        let other = match &mut self.inner {
            ...
            BufViewInner::JsBuffer(buffer) => BufViewInner::JsBuffer(buffer.split_off(at)),
        };
        Self {
            inner: other,
            cursor: 0,
    ...
    pub fn split_to(&mut self, at: usize) -> Self {
        ...
        let other = match &mut self.inner {
            ...
            BufViewInner::JsBuffer(buffer) => BufViewInner::JsBuffer(buffer.split_to(at)),
        };
...
impl BufMutView {
    ...
    #[doc(hidden)]
    pub fn maybe_resize(&mut self, target_size: usize, maximum_increment: usize) -> Option<usize> {
        match &mut self.inner {
    ...
    pub fn split_off(&mut self, at: usize) -> Self {
        ...
        assert!(at <= self.len());
        let other = match &mut self.inner {
            BufMutViewInner::Bytes(bytes) => BufMutViewInner::Bytes(bytes.split_off(at)),
            BufMutViewInner::JsBuffer(buffer) => BufMutViewInner::JsBuffer(buffer.split_off(at)),
        };
        Self {
            inner: other,
            cursor: 0,
    ...
    pub fn split_to(&mut self, at: usize) -> Self {
        ...
        let at = at + self.cursor;
        let other = match &mut self.inner {
            BufMutViewInner::Bytes(bytes) => BufMutViewInner::Bytes(bytes.split_to(at)),
            BufMutViewInner::JsBuffer(buffer) => BufMutViewInner::JsBuffer(buffer.split_to(at)),
        };
        let cursor = std::mem::take(&mut self.cursor);
        Self {
            inner: other,
...
mod tests {
    ...
    pub fn bufmutview_split() {
        let mut buf = BufMutView::from(BytesMut::from(Vec::from_iter(0..100).as_slice()));
        assert_eq!(100, buf.len());
        buf.advance_cursor(25);
        assert_eq!(75, buf.len());
        let mut other = buf.split_off(10);
    ...
    fn bufmutview_resize() {
        let new = || BufMutView::from(BytesMut::from(Vec::from_iter(0..100).as_slice()));
        let mut buf = new();
        assert_eq!(100, buf.len());
