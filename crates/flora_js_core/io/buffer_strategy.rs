...
impl AdaptiveBufferStrategy {
    ...
    pub fn notify_read(&mut self, nread: usize) {
        ...
        if nread >= self.grow_len && self.grow_len < MAX_GROW_LEN && self.expected_remaining > 0 {
            self.grow_len *= 2;
        }
    }
}
...
mod tests {
    ...
    impl ReadReporter for MaxReader {
        fn read(&mut self, n: usize) -> usize {
            let n = std::cmp::min(self.total_size, std::cmp::min(n, self.max_read_size));
    ...
    fn huge_hints_are_capped() {
        ...
        let strategy =
            AdaptiveBufferStrategy::new_from_size_class(SizeClass::BoundedMax(usize::MAX));
    ...
    fn huge_u64_hints_are_saturating_and_capped() {
        ...
        let strategy =
            AdaptiveBufferStrategy::new_from_hint_u64(oversized_hint, Some(oversized_hint));
