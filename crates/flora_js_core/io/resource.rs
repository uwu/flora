...
pub trait Resource: Any + 'static {
    ...
    fn read_byob_sync(self: Rc<Self>, data: &mut [u8]) -> Result<usize, JsErrorBox> {
        _ = data;
    ...
    (__write) => {
        fn write(
            ...
            ::std::boxed::Box::pin(async move {
                ...
                ::std::result::Result::Ok($crate::WriteOutcome::Partial { nwritten, view })
            })
        }
    };
    (__write_all) => {
        fn write_all(self: ::std::rc::Rc<Self>, view: $crate::BufView) -> $crate::AsyncResult<()> {
            ::std::boxed::Box::pin(async move {
                self.write_all(&view)
                    .await
                    .map_err(::deno_error::JsErrorBox::from_err)?;
