...
fn try_unwrap_cppgc_with<'sc, T: GarbageCollected + 'static>(
    ...
    let obj = unsafe { v8::Object::unwrap::<CPPGC_SINGLE_TAG, CppGcObject<T>>(isolate, obj) }?;
...
pub fn try_unwrap_cppgc_base_object<'sc, T: GarbageCollected + Base + 'static>(
    isolate: &mut v8::Isolate,
    val: v8::Local<'sc, v8::Value>,
) -> Option<UnsafePtr<T>> {
...
pub fn try_unwrap_cppgc_base_persistent_object<'sc, T: GarbageCollected + Base + 'static>(
    isolate: &mut v8::Isolate,
    val: v8::Local<'sc, v8::Value>,
) -> Option<Ref<T>> {
...
impl FunctionTemplateData {
    pub fn insert(&mut self, key: String, value: v8::Global<v8::FunctionTemplate>) {
    ...
    pub fn get_raw(&self, key: &str) -> Option<&v8::Global<v8::FunctionTemplate>> {
        self.store.get(key)
    }
...
impl<T: GarbageCollected + 'static> SameObject<T> {
    ...
    pub fn try_get<F, E>(&self, scope: &mut v8::PinScope, f: F) -> Result<v8::Global<v8::Object>, E>
    where
    ...
    pub fn set(&self, scope: &mut v8::PinScope, value: T) -> Result<(), v8::Global<v8::Object>> {
...
pub unsafe trait Base: GarbageCollected + 'static {
    ...
    fn inheriting_types() -> &'static [TypeId] {
        Self::__cache().get_or_init(|| find_transitive_inheritors(TypeId::of::<Self>()))
    }
}

pub const fn verify_inherits<Base: GarbageCollected + 'static, Derived: Inherits<Base>>()
-> InheritanceEdge {
    InheritanceEdge {
        base: TypeId::of::<Base>(),
