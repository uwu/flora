...
pub fn setup_op_state(op_state: &mut OpState, extensions: &mut [Extension]) -> Vec<&'static str> {
...
pub fn get_middlewares_and_external_refs(
    ...
    for extension in extensions {
        ...
        additional_references.extend_from_slice(extension.get_external_references());
    }
...
impl<'a> IntoIterator for &'a LoadedSources {
    ...
    type IntoIter = Chain<Chain<VecIntoIter<'a>, VecIntoIter<'a>>, VecIntoIter<'a>>;
...
impl<'a> IntoIterator for &'a mut LoadedSources {
    ...
    type IntoIter = Chain<Chain<VecIntoIterMut<'a>, VecIntoIterMut<'a>>, VecIntoIterMut<'a>>;
    fn into_iter(self) -> Self::IntoIter {
        self.js
            .iter_mut()
...
pub fn into_sources_and_source_maps(
    ...
    for (extension, extension_in_snapshot) in extensions.iter().zip(extensions_in_snapshot) {
        let snapshotted = if let Some(name) = extension_in_snapshot {
            if extension.name != *name {
                return Err(CoreErrorKind::ExtensionSnapshotMismatch(
        ...
        for file in &*extension.lazy_loaded_esm_files {
            ...
            let (code, maybe_source_map) = load(transpiler, file, &mut load_callback)?;
        ...
        for file in &*extension.lazy_loaded_js_files {
            ...
            let (mut code, maybe_source_map) = load(transpiler, file, &mut load_callback)?;
