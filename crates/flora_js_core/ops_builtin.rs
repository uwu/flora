...
#[op2(fast)]
pub fn op_close(state: Rc<RefCell<OpState>>, #[smi] rid: ResourceId) -> Result<(), ResourceError> {
...
#[op2(fast)]
pub fn op_print(#[string] msg: &str, is_err: bool) -> Result<(), std::io::Error> {
...
async fn op_read_all(
    ...
    let mut buffer_strategy = AdaptiveBufferStrategy::new_from_hint_u64(min, maybe_max);
...
async fn op_pipe(
    ...
    let dst = state
        ...
        .map_err(JsErrorBox::from_err)?;
    let cancel_handle =
        cancel_rid.and_then(|rid| state.borrow().resource_table.get::<CancelHandle>(rid).ok());
...
async fn do_load_job<'s, 'i>(
    ...
    match module.get_status() {
        ...
        v8::ModuleStatus::Errored => {
            return Err(CoreErrorKind::Js(exception_to_err(
                scope,
...
fn op_import_sync<'s, 'i>(
    ...
    let module_id =
        futures::executor::block_on(do_load_job(scope, module_map_rc.clone(), specifier, code))?;
...
fn op_import_sync_with_source<'s, 'i>(
    ...
    module_map_rc
        ...
        .map_err(|e| {
            let exception = v8::Local::new(scope, e);
            CoreErrorKind::Js(exception_to_err(scope, exception, false, false)).into_box()
        })?;
