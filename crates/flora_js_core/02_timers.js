...
'use strict'
;((window) => {
  const { MathMax, MathTrunc, NumberMIN_SAFE_INTEGER } = window.__bootstrap.primordials
  const {
    op_timer_schedule,
    op_timer_track,
    op_timer_untrack,
  ...
  const { ErrorCaptureStackTrace, StringPrototypeSlice } = window.__bootstrap.primordials
  ...
  function listOnTimeout(list, now) {
    ...
    while ((timer = L_peek(list)) != null) {
      ...
      try {
        ...
        if (timer._repeat && timer._idleTimeout !== -1 && !timer._idlePrev && !timer._idleNext) {
