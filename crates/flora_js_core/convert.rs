...
impl_number_types!(u8, i8, u16, i16, u32, i32, u64, i64, usize, isize, f32, f64);
...
impl<'s> FromV8<'s> for Uuid {
    ...
    fn from_v8<'i>(
        ...
        Uuid::parse_str(&s).map_err(|e| JsErrorBox::type_error(format!("Invalid UUID: {e}")))
    }
}
...
impl<'a> ToV8<'a> for ByteString {
    ...
    fn to_v8<'i>(
        ...
        let v =
            v8::String::new_from_one_byte(scope, self.as_ref(), v8::NewStringType::Normal).unwrap();
        Ok(v.into())
    }
}
...
unsafe fn abview_to_box<T>(ab_view: v8::Local<v8::ArrayBufferView>) -> Box<[T]> {
    ...
    unsafe {
        std::ptr::copy_nonoverlapping(data.cast::<T>(), out.as_mut_ptr().cast::<T>(), len);
        out.assume_init()
    }
}
...
impl<'a> ToV8<'a> for ArrayBufferView {
    ...
    fn to_v8<'i>(self, scope: &mut PinScope<'a, 'i>) -> Result<Local<'a, v8::Value>, Self::Error> {
        match self {
    ...
    S: BuildHasher,
{
    ...
    fn to_v8<'i>(
        ...
        let obj = v8::Object::with_prototype_and_properties(scope, null, &keys, &values);
        Ok(obj.into())
    }
}
    ...
    E: Into<V8ConvertError>,
{
    ...
    fn to_v8<'i>(self, scope: &mut PinScope<'s, 'i>) -> Result<Local<'s, v8::Value>, Self::Error> {
    ...
    E: Into<V8ConvertError>,
{
    ...
    fn to_v8<'i>(self, _scope: &mut PinScope<'s, 'i>) -> Result<Local<'s, v8::Value>, Self::Error> {
...
"#;
    ...
    fn key<'a>(scope: &mut v8::PinScope<'a, '_>, name: &str) -> v8::Local<'a, v8::Value> {
    ...
    fn test_option_undefined() {
        ...
        from_v8_test!(runtime, "undefined", |scope, result| {
            let r = <OptionUndefined<Number<f64>> as FromV8>::from_v8(scope, result).unwrap();
            assert_eq!(r, OptionUndefined(None));
        });

        from_v8_test!(runtime, "1.0", |scope, result| {
            assert_eq!(
                <OptionUndefined::<Number<f64>> as FromV8>::from_v8(scope, result).unwrap(),
