...
enum V8Isolate {
    ...
    Shared(v8::SharedIsolate),
}

impl V8Isolate {
    fn guard(&mut self) -> V8Guard<'_> {
        match self {
            Self::Owned(isolate) => V8Guard(V8GuardInner::Owned(isolate)),
            Self::Shared(isolate) => V8Guard(V8GuardInner::Shared(isolate.lock())),
        }
    }
}

enum V8GuardInner<'a> {
    Owned(&'a mut v8::OwnedIsolate),
    Shared(v8::Locker<'a>),
}

pub struct V8Guard<'a>(V8GuardInner<'a>);

impl V8Guard<'_> {
    pub fn isolate(&mut self) -> &mut v8::Isolate {
        match &mut self.0 {
            V8GuardInner::Owned(isolate) => isolate,
            V8GuardInner::Shared(locker) => locker,
        }
    }
}

impl Deref for V8Guard<'_> {
...
pub(crate) struct InnerIsolateState {
    ...
    v8_isolate: Option<V8Isolate>,
}

impl InnerIsolateState {
    fn runtime_parts(&mut self) -> (&mut V8Isolate, &JsRealm, &JsRuntimeState) {
        let isolate = self
            .v8_isolate
            .as_mut()
            .expect("V8 isolate must be available");
        (isolate, &self.main_realm, &self.state)
    }
    ...
    pub fn prepare_for_cleanup(&mut self) {
        let isolate = self
            .v8_isolate
            .as_mut()
            .expect("V8 isolate must be available during cleanup");
        let _guard = isolate.guard();
    ...
    pub fn cleanup(&mut self) {
        ...
        unsafe {
            ManuallyDrop::take(main_realm).0.destroy(guard.isolate());
        }
        ...
        debug_assert_eq!(Rc::strong_count(state), 1);
    }
    ...
    fn drop_state(&mut self) {
        ...
        let isolate = v8_isolate
            .as_mut()
            .expect("V8 isolate must be available while dropping runtime state");
        let _guard = isolate.guard();
        unsafe {
            ManuallyDrop::drop(&mut state.0);
        }
    }
    ...
    pub fn prepare_for_snapshot(mut self) -> v8::OwnedIsolate {
        ...
        let isolate = self
            .v8_isolate
            .take()
            .expect("V8 isolate must be available for snapshot");
        let V8Isolate::Owned(isolate) = isolate else {
            panic!("snapshot runtimes cannot use a shared V8 isolate");
        };
        std::mem::forget(self);
        isolate
    }
...
impl Drop for InnerIsolateState {
    fn drop(&mut self) {
        ...
        let isolate = self
            .v8_isolate
            .take()
            .expect("V8 isolate must be available during drop");
...
pub struct JsRuntimeState {
    ...
    pub(crate) vm_dynamic_import_callbacks: RefCell<HashMap<u32, v8::Global<v8::Function>>>,
    ...
    pub(crate) external_module_import_meta_cb: Cell<Option<ExternalModuleImportMetaCb>>,
...
pub struct RuntimeOptions {
    ...
    /// Convert the runtime to a shared V8 isolate guarded by `v8::Locker`.
    ///
    /// Setting this asserts that all extension state attached to the isolate,
    /// including state attached after construction, can be moved between
    /// threads and accessed by one thread at a time.
    ///
    /// Cross-thread use also requires an unprotected V8 platform, such as the
    /// one selected by the `unsafe_use_unprotected_platform` feature.
    pub use_v8_locker: bool,
}
...
macro_rules! scope {
    ($scope: ident, $self: expr) => {
        ...
        let mut __v8_guard = $self.v8_guard();
        let isolate = __v8_guard.isolate();
...
impl JsRuntime {
    ...
    /// this may expose the runtime to security vulnerabilities.
    #[cfg(any(test, feature = "unsafe_runtime_options"))]
    pub fn init_platform(v8_platform: Option<v8::SharedRef<v8::Platform>>, expose_natives: bool) {
    ...
    fn new_inner(mut options: RuntimeOptions, will_snapshot: bool) -> Result<JsRuntime, CoreError> {
        ...
        let use_v8_locker = options.use_v8_locker;
        ...
        ) = bindings::externalize_sources(&mut sources, snapshot_sources, externalize_lazy_js);
        ...
        let main_realm = {
            ...
            if init_mode.needs_ops_bindings() {
                ...
                let (ops_obj, stub_fn) = bindings::snapshotted_fast_op_refs(scope, context);
            ...
            unsafe {
                super::jsrealm::initialize_context_data(&context);
                super::jsrealm::set_context_data(
                    &context,
        ...
        let mut js_runtime = JsRuntime {
            inner: InnerIsolateState {
                ...
                isolate_ptr,
                v8_isolate: Some(V8Isolate::Owned(isolate)),
            },
        ...
        if !will_snapshot {
            ...
            let mut uv_loop = Box::new(unsafe { std::mem::zeroed::<uv_compat::UvLoop>() });
        ...
        if use_v8_locker {
            if will_snapshot {
                return Err(CoreError::from(JsErrorBox::generic(
                    "snapshot runtimes cannot use a shared V8 isolate",
                )));
            }
            js_runtime.enable_v8_locker()?;
        }
    ...
    pub fn lazy_init_extensions(&self, ext_args: Vec<ExtensionArguments>) -> Result<(), CoreError> {
        if ext_args.len() != self.inner.state.lazy_extensions.len() {
            return Err(CoreErrorKind::ExtensionLazyInitCountMismatch(
            ...
            .zip(self.inner.state.lazy_extensions.iter())
        {
            if args.name != *expected_name {
                return Err(CoreErrorKind::ExtensionLazyInitOrderMismatch(
    ...
    #[inline]
    pub fn main_context(&mut self) -> v8::Global<v8::Context> {
        let (isolate, realm, _) = self.inner.runtime_parts();
        let _guard = isolate.guard();
    ...
    pub fn v8_isolate(&mut self) -> &mut v8::OwnedIsolate {
        let Some(V8Isolate::Owned(isolate)) = self.inner.v8_isolate.as_mut() else {
            panic!("shared V8 isolates must be accessed through JsRuntime::v8_guard")
        };
        isolate
    }

    #[inline]
    pub fn v8_guard(&mut self) -> V8Guard<'_> {
        self.inner
            .v8_isolate
            .as_mut()
            .expect("V8 isolate must be available")
    ...
    pub fn thread_safe_handle(&self) -> v8::IsolateHandle {
        let isolate = self
            ...
            .expect("V8 isolate must be available");
        match isolate {
            V8Isolate::Owned(isolate) => isolate.thread_safe_handle(),
            V8Isolate::Shared(isolate) => isolate.thread_safe_handle(),
        }
    }

    fn enable_v8_locker(&mut self) -> Result<(), CoreError> {
        let isolate = self
            .inner
            .v8_isolate
            .take()
            .expect("V8 isolate must be available");
        let V8Isolate::Owned(isolate) = isolate else {
            self.inner.v8_isolate = Some(isolate);
            return Ok(());
        };

        // SAFETY: `RuntimeOptions::use_v8_locker` requires the embedder to make
        // all isolate and extension state safe to move between threads. Flora
        // serializes access by transferring a stopped runtime between workers.
        match unsafe { isolate.try_into_shared() } {
            Ok(isolate) => {
                self.inner.v8_isolate = Some(V8Isolate::Shared(isolate));
                Ok(())
            }
            Err(error) => {
                let message = error.kind().to_string();
                self.inner.v8_isolate = Some(V8Isolate::Owned(error.into_isolate()));
                Err(CoreError::from(JsErrorBox::generic(format!(
                    "failed to convert V8 isolate for Locker access: {message}"
                ))))
            }
        }
    }
    ...
    async fn init_extension_js_inner(
        ...
        *module_map.loader.borrow_mut() = ext_loader.clone();
        ...
        for module in modules {
            let mut guard = self.v8_guard();
            realm
                .load_side_es_module_from_code(guard.isolate(), module.into(), None)
                .await?;
        }
        ...
        for source in loaded_sources.js {
            match &ext_code_cache {
                ...
                _ => {
                    let mut guard = self.v8_guard();
                    realm.execute_script(guard.isolate(), source.specifier, source.code)?;
                }
            }
        }
        ...
        for specifier in loaded_sources.esm_entry_points {
            let Some(mod_id) = module_map.get_id(&specifier, RequestedModuleType::None) else {
                return Err(CoreErrorKind::MissingFromModuleMap(specifier.to_string()).into_box());
            };

            let mut guard = self.v8_guard();
            jsrealm::context_scope!(scope, realm, guard.isolate());
    ...
    pub fn eval<'s, 'i, T>(scope: &mut v8::PinScope<'s, 'i>, code: &str) -> Option<v8::Local<'s, T>>
    where
    ...
    fn store_js_callbacks(&mut self, realm: &JsRealm, will_snapshot: bool) {
        let (
            ...
            let process_timers_cb: v8::Local<v8::Function> =
                bindings::get(scope, core_obj, PROCESS_TIMERS, "Deno.core.__processTimers");
            let drain_next_tick_and_macrotasks_cb: v8::Local<v8::Function> = bindings::get(
                scope,
                core_obj,
            ...
            // hasTickScheduled without crossing the JS-to-Rust boundary.
            {
                ...
                let tick_info_ptr = state_rc.tick_info.as_ptr() as *mut std::ffi::c_void;
                ...
                let set_tick_info_fn: v8::Local<v8::Function> =
                    bindings::get(scope, core_obj, SET_TICK_INFO, "Deno.core.__setTickInfo");
            ...
            {
                let state_rc = realm.0.state();
                let imm_info_ptr = state_rc.immediate_info.as_ptr() as *mut std::ffi::c_void;
            ...
            {
                let state_rc = realm.0.state();
                let timer_info_ptr = state_rc.timer_info.as_ptr() as *mut std::ffi::c_void;
                ...
                let set_timer_info_fn: v8::Local<v8::Function> =
                    bindings::get(scope, core_obj, SET_TIMER_INFO, "Deno.core.__setTimerInfo");
                let undefined: v8::Local<v8::Value> = v8::undefined(scope).into();
    ...
    pub unsafe fn register_uv_loop(&mut self, loop_ptr: *mut uv_compat::uv_loop_t) {
        let (isolate, realm, _) = self.inner.runtime_parts();
        let _guard = isolate.guard();
    ...
    pub fn get_module_namespace(
        ...
        let (isolate, realm, _) = self.inner.runtime_parts();
        let mut guard = isolate.guard();
        realm.get_module_namespace(guard.isolate(), module_id)
    }
        ...
        C: FnMut(usize, usize) -> usize + 'static,
    {
        ...
        if let Some((_, prev_cb)) = prev {
            let mut guard = self.v8_guard();
            guard.remove_near_heap_limit_callback(prev_cb, 0);
        }

        let mut guard = self.v8_guard();
        guard.add_near_heap_limit_callback(near_heap_limit_callback::<C>, data);
    }

    pub fn remove_near_heap_limit_callback(&mut self, heap_limit: usize) {
        if let Some((_, cb)) = self.allocations.near_heap_limit_callback_data.take() {
            let mut guard = self.v8_guard();
            guard.remove_near_heap_limit_callback(cb, heap_limit);
        }
    }

    pub fn maybe_init_inspector(&mut self) {
        if self.inner.state.inspector.borrow().is_some() {
            return;
        }

        let context = self.main_context();
        let isolate_ptr = self.inner.isolate_ptr;
        let is_main_runtime = self.is_main_runtime;
        let inspector = {
            let mut guard = self.v8_guard();
            v8::scope_with_context!(scope, guard.isolate(), &context);
            let context = v8::Local::new(scope, &context);
            JsRuntimeInspector::new(isolate_ptr, scope, context, is_main_runtime, None)
        };

        self.inner.state.has_inspector.set(true);
        *self.inner.state.inspector.borrow_mut() = Some(inspector);
    }
        ...
        CoreError: From<E>,
    {
        ...
        poll_fn(|cx| {
            ...
            if let Poll::Ready(t) = self.poll_event_loop(cx, poll_options) {
                ...
                return Poll::Ready(Err(CoreErrorKind::PendingPromiseResolution.into_box()));
            }
            Poll::Pending
        })
        .await
    ...
    pub fn poll_event_loop(
        ...
        let (isolate, realm, state) = self.inner.runtime_parts();
        let has_inspector = state.has_inspector.get();
        let mut guard = isolate.guard();

        let result = {
            v8::scope!(let isolate_scope, guard.isolate());
            let context = v8::Local::new(isolate_scope, realm.context());
            let mut scope = v8::ContextScope::new(isolate_scope, context);
            Self::poll_event_loop_inner(state, realm, cx, &mut scope, poll_options)
        };
        ...
        if has_inspector {
            guard.set_idle(result.is_pending());
        }
        result
    }

    pub fn is_idle_for_migration(&mut self) -> bool {
        let (isolate, realm, _) = self.inner.runtime_parts();
        let mut guard = isolate.guard();
        v8::scope!(let isolate_scope, guard.isolate());
        let context = v8::Local::new(isolate_scope, realm.context());
        let mut scope = v8::ContextScope::new(isolate_scope, context);
        let pending =
            EventLoopPendingState::new(&mut scope, &realm.0.context_state, &realm.0.module_map);
        pending.is_idle_for_migration()
    }
    ...
    fn poll_event_loop_inner(
        ...
        if has_inspector {
            state.inspector().poll_sessions_from_event_loop(cx);
        }
        {
            ...
            let tasks = std::mem::take(&mut *state.foreground_tasks.lock().unwrap());
            for task in tasks {
                task.run();
            }
            ...
            if let Some(exception) = tc_scope.exception() {
                return Poll::Ready(Err(exception_to_err_result::<()>(
        ...
        {
            let has_immediates = context_state.immediate_info[IMM_IDX_HAS_OUTSTANDING] != 0
                || context_state.immediate_info[IMM_IDX_COUNT] > 0;
            let has_refed = context_state.immediate_info[IMM_IDX_REF_COUNT] > 0;
            if has_immediates && (has_refed || did_work || dispatched_ops || uv_did_io) {
            ...
            if has_inspector {
                ...
                if poll_options.wait_for_inspector && sessions_state.has_active {
                    ...
                    if sessions_state.has_nonblocking_wait_for_disconnect {
                        let context = realm.0.context().clone();
                        inspector.context_destroyed(scope, context);
                        if let Some(callback) =
                            state.wait_for_inspector_disconnect_callback.as_ref()
                        {
                            callback();
                        }
                        return Poll::Pending;
        ...
        if pending_state.has_pending_module_evaluation {
                ...
                || pending_state.has_uv_alive_handles
            {
                    ...
                    find_and_report_stalled_level_await_in_any_realm(scope, &realm.0)
                {
                    ...
                    let retries = state.tla_stall_retries.get() + 1;
                    state.tla_stall_retries.set(retries);
                    if retries > MAX_TLA_STALL_RETRIES {
                        state.tla_stall_retries.set(0);
                        return Poll::Ready(
                            Err(CoreErrorKind::ModuleEvaluationDeadlock.into_box()),
                        );
                    }
                    #[allow(
                        clippy::print_stderr,
...
impl JsRuntimeForSnapshot {
    ...
    pub fn snapshot(mut self) -> Box<[u8]> {
        ...
        let lazy_source_specifiers = std::mem::take(&mut self.0.allocations.lazy_source_specifiers);
        ...
        let lazy_source_start = original_sources.len() - lazy_source_specifiers.len();
        let external_strings = original_sources
            .iter()
            .enumerate()
            .map(|(i, s)| {
                if i >= lazy_source_start
                    && !consumed.contains(lazy_source_specifiers[i - lazy_source_start].as_str())
                {
                    return &[][..];
                }
                s.as_str().as_bytes()
...
impl EventLoopPendingState {
    ...
    pub fn new(scope: &mut v8::PinScope<()>, state: &ContextState, modules: &ModuleMap) -> Self {
        ...
        let has_pending_refed_ops =
            has_pending_tasks || has_pending_refed_user_timers || num_pending_ops > num_unrefed_ops;
        let has_outstanding_immediates = state.immediate_info[IMM_IDX_HAS_OUTSTANDING] != 0;
        let has_pending_timers = !state.active_timers.borrow().is_empty();
        let has_uv_alive_handles = if let Some(uv_inner_ptr) = state.uv_loop_inner.get() {
    ...
    fn is_idle_for_migration(&self) -> bool {
...
impl JsRuntime {
    ...
    pub fn mod_evaluate(
        ...
        let mut guard = isolate.guard();
        jsrealm::context_scope!(scope, realm, guard.isolate());
        realm.0.module_map.mod_evaluate(scope, id)
    }
    ...
    pub async fn load_main_es_module_from_code(
        ...
        self.drive_es_module_load(true, specifier, Some(code.into_module_code()))
            .await
    }
    ...
    pub async fn load_side_es_module_from_code(
        ...
        self.drive_es_module_load(false, specifier, Some(code.into_module_code()))
            .await
    }
    ...
    async fn drive_es_module_load(
        ...
        let pump_event_loop = module_map_rc.loader.borrow().pump_event_loop_during_load();
        ...
        let mut load = if is_main {
            RecursiveModuleLoad::main(specifier.to_string(), module_map_rc.clone()).await?
        } else {
        ...
        loop {
            ...
            let mut guard = self.v8_guard();
            jsrealm::context_scope!(scope, &realm, guard.isolate());
            load.register_and_recurse(scope, &request, source)
                .map_err(|e| e.into_error(scope, false, false))?;
        }
    ...
    pub fn lazy_load_es_module_with_code(
        ...
        let (isolate, realm, _) = self.inner.runtime_parts();
        let mut guard = isolate.guard();
        realm.lazy_load_es_module_with_code(
            guard.isolate(),
    ...
    fn do_js_run_immediate_callbacks<'s, 'i>(
        ...
        let run_immediate_callbacks_cb = context_state.run_immediate_callbacks_cb.borrow();
    ...
    fn dispatch_rejections<'s, 'i>(
        ...
        let handle_rejections_fn = v8::Local::new(tc_scope, handle_rejections_cb.as_ref().unwrap());
        let undefined: v8::Local<v8::Value> = v8::undefined(tc_scope).into();
    ...
    fn drain_next_tick_and_macrotasks<'s, 'i>(
        ...
        if !context_state.has_tick_scheduled() && !context_state.has_rejection_to_warn() {
            scope.perform_microtask_checkpoint();
            if !context_state.has_tick_scheduled() && !context_state.has_rejection_to_warn() {
                return Ok(());
            }
