...
const NO_TRACES: [BTreeMap<ActivityId, Rc<str>>; RuntimeActivityType::MAX_TYPE as usize] = [
    BTreeMap::new(),
    BTreeMap::new(),
    BTreeMap::new(),
    BTreeMap::new(),
...
pub struct RuntimeActivityTraces {
    enabled: Cell<bool>,
    traces: RefCell<[BTreeMap<ActivityId, Rc<str>>; RuntimeActivityType::MAX_TYPE as usize]>,
}
...
impl RuntimeActivityTraces {
    ...
    pub(crate) fn submit(&self, activity_type: RuntimeActivityType, id: ActivityId, trace: &str) {
        debug_assert_ne!(
            activity_type,
    ...
    pub(crate) fn complete(&self, activity_type: RuntimeActivityType, id: ActivityId) {
    ...
    pub fn get_all(&self, mut f: impl FnMut(RuntimeActivityType, ActivityId, &str)) {
...
impl RuntimeActivityStatsFactory {
    ...
    pub fn capture(self, filter: &RuntimeActivityStatsFilter) -> RuntimeActivityStats {
...
impl RuntimeActivityStats {
    ...
    pub fn dump(&self) -> RuntimeActivitySnapshot {
        ...
        let mut v = Vec::with_capacity(
            self.ops.ops.len() + self.resources.resources.len() + self.timers.timers.len(),
        );
