...
const V8_CONTEXT_ANNEX_SLOT_INDEX: i32 = 1;
const V8_CONTEXT_INTERNAL_SLOT_COUNT: i32 = 2;
...
pub(crate) unsafe fn initialize_context_data(context: &v8::Context) {
    unsafe {
        v8__Context__SetAlignedPointerInEmbedderData(
            context,
            V8_CONTEXT_ANNEX_SLOT_INDEX,
            std::ptr::null_mut(),
        );
    }
}

pub(crate) unsafe fn get_context_data(context: &v8::Context, slot: i32) -> *mut c_void {
    unsafe {
        v8__Context__GetAlignedPointerFromEmbedderData(
            context,
            slot + V8_CONTEXT_INTERNAL_SLOT_COUNT,
        )
    }
}

pub(crate) unsafe fn set_context_data(context: &v8::Context, slot: i32, value: *mut c_void) {
    unsafe {
        v8__Context__SetAlignedPointerInEmbedderData(
            context,
            slot + V8_CONTEXT_INTERNAL_SLOT_COUNT,
...
pub struct ContextState {
    ...
    pub(crate) js_drain_next_tick_and_macrotasks_cb: RefCell<Option<v8::Global<v8::Function>>>,
    pub(crate) js_handle_rejections_cb: RefCell<Option<v8::Global<v8::Function>>>,
    pub(crate) run_immediate_callbacks_cb: RefCell<Option<v8::Global<v8::Function>>>,
    ...
    pub(crate) active_timers: RefCell<std::collections::HashMap<usize, (bool, bool)>>,
    ...
    pub(crate) webidl_sequence_keys: RefCell<Option<crate::webidl::WebIdlSequenceKeys>>,
    ...
    pub(crate) immediate_check_handle: RefCell<Option<crate::uv_compat::ImmediateCheckHandle>>,
}
...
impl JsRealmInner {
    ...
    pub fn destroy(self, isolate: &mut v8::Isolate) {
...

        {
            ...
            unsafe {
                let ctx_state = get_context_data(&ctx, CONTEXT_STATE_SLOT_INDEX);
                let _ = Rc::from_raw(ctx_state as *mut ContextState);

                let module_map = get_context_data(&ctx, MODULE_MAP_SLOT_INDEX);
                ...
                set_context_data(&ctx, CONTEXT_STATE_SLOT_INDEX, std::ptr::null_mut());
                set_context_data(&ctx, MODULE_MAP_SLOT_INDEX, std::ptr::null_mut());
            }
            ctx.clear_all_slots();
...
impl JsRealm {
    ...
    pub(crate) fn state_from_scope(scope: &mut v8::PinScope) -> Rc<ContextState> {
        ...
        unsafe {
            let rc = get_context_data(&context, CONTEXT_STATE_SLOT_INDEX);
    ...
    pub(crate) fn module_map_from(scope: &mut v8::PinScope) -> Rc<ModuleMap> {
        ...
        // SAFETY: slot is valid and set during realm creation
        unsafe {
            let rc = get_context_data(&context, MODULE_MAP_SLOT_INDEX);
            clone_rc_raw(rc as *const ModuleMap)
        }
    }

    #[inline(always)]
    pub(crate) fn exception_state_from_scope(scope: &mut v8::PinScope) -> Rc<ExceptionState> {
    ...
    pub(crate) async fn load_side_es_module_from_code(
        ...
        .run_to_completion(|load, step| {
            context_scope!(scope, self, isolate);
            match step {
                crate::modules::recursive_load::RegisterStep::Register { request, source } => load
