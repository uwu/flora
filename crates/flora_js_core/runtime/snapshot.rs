...
pub(crate) fn deconstruct(
    ...
    let len = usize::from_le_bytes(slice[slice.len() - ULEN..].try_into().unwrap());
    let data = SerializableSnapshotSidecarData::from_slice(&slice[len..slice.len() - ULEN]);
...
impl SnapshotStoreDataStore {
        ...
        for<'s> v8::Local<'s, v8::Data>: From<v8::Local<'s, T>>,
    {
        ...
        unsafe {
            self.data
                .push(std::mem::transmute::<v8::Global<T>, v8::Global<v8::Data>>(
                    global,
                ));
        }
        id as _
