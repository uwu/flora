...
pub(crate) fn externalize_sources(
    ...
    const INIT_VALUE: MaybeUninit<[usize; 3]> = MaybeUninit::<[usize; 3]>::uninit();
    ...
    let externals = vec![INIT_VALUE; sources_count + snapshot_sources.len()].into_boxed_slice();
    ...
    unsafe {
        ...
        for (index, source) in snapshot_sources.iter().enumerate() {
            externals[index + offset] =
                FastStaticString::create_external_onebyte_const_unchecked_in_release(source);
        }
        ...
        for (index, source) in source_iter.enumerate() {
            ...
            let original_source =
                std::mem::replace(&mut source.code, FastStaticString::from(&*ptr).into());
...
pub(crate) fn initialize_primordials_and_infra(scope: &mut v8::PinScope) -> Result<(), CoreError> {
    for source_file in &CONTEXT_SETUP_SOURCES {
...
fn name_key<'s, 'i>(scope: &mut v8::PinScope<'s, 'i>, decl: &OpDecl) -> v8::Local<'s, v8::Name> {
...
pub(crate) fn initialize_deno_core_ops_bindings<'s, 'i>(
    ...
    let deno_core_ops_obj: v8::Local<v8::Object> = get(scope, deno_core_obj, OPS, "Deno.core.ops");
    ...
    for decl in op_method_decls {
        ...
        if !will_snapshot {
            for method in static_method_ctxs.iter() {
                let op_fn =
                    op_ctx_template(scope, method, v8::ConstructorBehavior::Throw, will_snapshot);
...
pub(crate) fn upgrade_snapshotted_ops_with_fast_calls<'s, 'i>(
    ...
    for decl in op_method_decls {
        ...
        let class_fn = class_fn_val.and_then(|v| v8::Local::<v8::Function>::try_from(v).ok());
        ...
        for method in method_ctxs {
            ...
            let method_fn = op_ctx_function(scope, method, v8::ConstructorBehavior::Throw, false);
            let method_key = name_key(scope, &method.decl);
            if method.decl.is_async {
...
fn op_ctx_template_or_accessor<'s, 'i>(
    ...
    if !op_ctx.decl.is_accessor() {
        let op_fn = op_ctx_template(scope, op_ctx, v8::ConstructorBehavior::Throw, will_snapshot);
    ...
    if let Some((named_getter, named_setter)) = accessor_store.get(op_ctx.decl.name) {
...
fn op_ctx_function_or_accessor<'s, 'i>(
    ...
    if !op_ctx.decl.is_accessor() {
        let op_fn = op_ctx_plain_function(scope, op_ctx, v8::ConstructorBehavior::Throw);
        let method_key = name_key(scope, &op_ctx.decl);
        if op_ctx.decl.is_async {
            let undefined = v8::undefined(scope);
            let _result = set_up_async_stub_fn
    ...
    if let Some((named_getter, named_setter)) = accessor_store.get(op_ctx.decl.name) {
        ...
        let getter_fn: v8::Local<v8::Value> = if let Some(getter) = named_getter {
            let op_fn =
                op_ctx_plain_function_with_length(scope, getter, v8::ConstructorBehavior::Throw, 0);
            let method_name = format!("get {}", op_ctx.decl.name_fast);
            let method_name = v8::String::new(scope, method_name.as_str()).unwrap();
            op_fn.set_name(method_name);
        ...
        let setter_fn: v8::Local<v8::Value> = if let Some(setter) = named_setter {
            let op_fn =
                op_ctx_plain_function_with_length(scope, setter, v8::ConstructorBehavior::Throw, 1);
            let method_name = format!("set {}", op_ctx.decl.name_fast);
            let method_name = v8::String::new(scope, method_name.as_str()).unwrap();
            op_fn.set_name(method_name);
        ...
        let key = op_ctx.decl.name_fast.v8_string(scope).unwrap();
        let mut desc = v8::PropertyDescriptor::new_from_get_set(getter_fn, setter_fn);
...
pub(crate) fn op_ctx_template<'s, 'i>(
        ...
        && !op_ctx.decl.constructable
    {
        ...
        let overloads: &'static [v8::fast_api::CFunction] =
            unsafe { std::mem::transmute::<&[_], &'static [_]>(overloads.as_slice()) };
...
fn op_ctx_function<'s, 'i>(
    ...
    let template = op_ctx_template(scope, op_ctx, constructor_behaviour, will_snapshot);
...
type AccessorStore<'a> = HashMap<String, (Option<&'a OpCtx>, Option<&'a OpCtx>)>;
    ...
    for method in method_ctxs.iter() {
        ...
        if method.decl.accessor_type == AccessorType::Setter {
            ...
            let getter = method_ctxs
                .iter()
                .find(|m| m.decl.name == key_str && m.decl.accessor_type == AccessorType::Getter);
...
pub fn host_import_module_with_phase_dynamically_callback<'s, 'i>(
    ...
    let requested_module_type = get_requested_module_type_from_attributes(&assertions);
    ...
    let map_err = v8::FunctionBuilder::<v8::Function>::build(builder, tc_scope).unwrap();
...
fn host_import_module_with_vm_dynamic_import_callback<'s, 'i>(
    ...
    let Some(callback_id) = crate::runtime::host_defined_options::read_host_defined_options_key(
        scope,
        host_defined_options,
    ) else {
        let exception = vm_dynamic_import_callback_missing_exception(scope);
...
fn vm_dynamic_import_callback_missing_exception<'s>(
    ...
    let code_key = v8::String::new_external_onebyte_static(scope, b"code").unwrap();
    let code_val =
        v8::String::new_external_onebyte_static(scope, b"ERR_VM_DYNAMIC_IMPORT_CALLBACK_MISSING")
            .unwrap();
...
pub extern "C" fn host_initialize_import_meta_object_callback(
    ...
    if module_type == ModuleType::Wasm {
        ...
        match state.wasm_instance_fn.borrow().as_ref() {
            ...
            _ => {
                let message =
                    v8::String::new(scope, "WebAssembly is not available in this environment")
                        .unwrap();
        ...
        if let Some(m) = state.wasm_instances_map.borrow().as_ref() {
            ...
            meta.create_data_property(scope, wasm_instances_key.into(), wasm_instances_val.into());
        }
    }
...
fn catch_dynamic_import_promise_error<'s, 'i>(
    ...
    if is_instance_of_error(scope, arg) {
        let e: crate::error::NativeJsError = serde_v8::from_v8(scope, arg).unwrap_or_default();
        let name = e
            .name
            .unwrap_or_else(|| deno_error::builtin_classes::GENERIC_ERROR.to_string());
        if !has_call_site(scope, arg) {
            ...
            {
                let str = format!("{} at {location}", message.to_rust_string_lossy(scope));
