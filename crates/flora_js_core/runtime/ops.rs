...
pub fn to_string_ptr(string: &v8::fast_api::FastApiOneByteString) -> String {
    ...
    unsafe {
        ...
        let written = v8::latin1_to_utf8(input_buf.len(), input_buf.as_ptr(), buffer.as_mut_ptr());
...
pub fn to_cow_byte_ptr(string: &v8::fast_api::FastApiOneByteString) -> Cow<'_, [u8]> {
...
pub fn v8_try_convert<'a, T>(value: v8::Local<'a, v8::Value>) -> Result<v8::Local<'a, T>, ()>
where
    ...
    v8::Local<'a, v8::ArrayBufferView>: From<v8::Local<'a, T::V8>>,
{
    ...
    let slice = unsafe { serde_v8::V8Slice::from_parts(store, offset..(offset + length)) };
    Ok(slice)
}
...
/// [`v8::ArrayBuffer`].
pub unsafe fn to_slice_buffer(input: v8::Local<'_, v8::Value>) -> Result<&mut [u8], &'static str> {
    unsafe {
...
pub fn to_v8_slice_buffer(
    ...
    let slice =
        unsafe { serde_v8::V8Slice::from_parts(buf.get_backing_store(), 0..buf.byte_length()) };
    Ok(slice)
}
...
pub fn to_v8_slice_buffer_detachable(
    ...
    let (store, length) = if let Ok(buf) = v8::Local::<v8::ArrayBuffer>::try_from(input) {
...
/// Retrieve a [`serde_v8::V8Slice`] from a [`v8::ArrayBuffer`].
pub fn to_v8_slice_any(input: v8::Local<v8::Value>) -> Result<serde_v8::V8Slice<u8>, &'static str> {
    if let Ok(buf) = v8::Local::<v8::ArrayBufferView>::try_from(input) {
...
mod tests {
    ...
    fn run_test2(repeat: usize, op: &str, test: &str) -> Result<(), CoreError> {
        ...
        let err_mapper = |err| JsErrorBox::generic(format!("{op} test failed ({test}): {err:?}"));
        runtime
            .execute_script(
                "",
                format!(
    ...
    /// Run a test for a single op.
    async fn run_async_test(repeat: usize, op: &str, test: &str) -> Result<(), anyhow::Error> {
        let mut runtime = JsRuntime::new(RuntimeOptions {
            extensions: vec![testing::init()],
            ..Default::default()
        });
    ...
    #[op2(fast)]
    pub fn op_test_string_roundtrip_char_onebyte(#[string(onebyte)] s: Cow<[u8]>) -> u32 {
        s[0] as u32
    }
    ...
    #[op2]
    #[string]
    pub fn op_test_string_return(#[string] a: Cow<str>, #[string] b: Cow<str>) -> String {
        (a + b).to_string()
    }
    ...
    #[op2]
    #[string(onebyte)]
    pub fn op_test_string_roundtrip_onebyte(#[string(onebyte)] s: Cow<[u8]>) -> Cow<[u8]> {
        s
    }
    ...
    pub fn op_test_v8_type_return<'s>(s: v8::Local<'s, v8::String>) -> v8::Local<'s, v8::String> {
        s
    }
    ...
    #[op2]
    #[cppgc]
    pub fn op_test_make_cppgc_resource_option(create: bool) -> Option<TestResource> {
        if create {
    ...
    #[op2]
    #[smi]
    pub async fn op_test_get_cppgc_resource(#[cppgc] resource: &TestResource) -> u32 {
    ...
    #[op2(fast)]
    #[smi]
    pub fn op_test_get_cppgc_resource_option(#[cppgc] resource: Option<&TestResource>) -> u32 {
    ...
    #[op2(fast)]
    fn op_external_process(input: *const std::ffi::c_void) -> *const std::ffi::c_void {
    ...
    #[op2(nofast)]
    fn op_isolate_queue_microtask(isolate: &mut v8::Isolate, cb: v8::Local<'_, v8::Function>) {
    ...
    pub async fn test_op_async_void() -> Result<(), Box<dyn std::error::Error>> {
        run_async_test(JIT_ITERATIONS, "op_async_void", "await op_async_void()").await?;
        Ok(())
    }
    ...
    pub async fn test_op_async_sleep() -> Result<(), Box<dyn std::error::Error>> {
        ...
        run_async_test(5, "op_async_sleep_impl", "await op_async_sleep_impl()").await?;
        Ok(())
    }
    ...
    pub async fn test_op_async_result_impl() -> Result<(), Box<dyn std::error::Error>> {
        ...
        run_async_test(5, "op_async_result_impl", "await op_async_result_impl(3);").await?;
        Ok(())
    }

    #[op2]
    pub async fn op_async_state_rc(state: Rc<RefCell<OpState>>, value: u32) -> u32 {
        let old_value: u32 = state.borrow_mut().take();
        state.borrow_mut().put(value);
        old_value
    }
    ...
    #[op2]
    fn op_async_buffer_impl(#[buffer] input: &[u8]) -> impl Future<Output = u32> + use<> {
        let l = input.len();
        async move { l as _ }
    }
    ...
    #[op2]
    async fn op_async_external(input: *const std::ffi::c_void) -> *const std::ffi::c_void {
        assert_eq!(input, STRING.as_ptr() as _);
        input
    }
    ...
    #[tokio::test]
    pub async fn test_op_bool_to_from_v8_error() -> Result<(), Box<dyn std::error::Error>> {
        let err = run_test2(
            JIT_ITERATIONS,
            "op_bool_to_from_v8",
            r#"
