...
impl<const MAX_SIZE: usize> TypeErased<MAX_SIZE> {
    ...
    pub unsafe fn raw_ptr<R>(this: *mut Self) -> NonNull<R> {
        unsafe { NonNull::new_unchecked(std::ptr::addr_of_mut!((*this).memory) as *mut _) }
    }
...
pub struct ErasedFuture<const MAX_SIZE: usize, Output> {
    ...
    poll: unsafe fn(this: Pin<&mut TypeErased<MAX_SIZE>>, cx: &mut Context) -> Poll<Output>,
    _output: PhantomData<Output>,
}

impl<const MAX_SIZE: usize, Output> ErasedFuture<MAX_SIZE, Output> {
    #[allow(dead_code, reason = "used in tests and related modules")]
    unsafe fn poll<F>(pin: Pin<&mut TypeErased<MAX_SIZE>>, cx: &mut Context<'_>) -> Poll<Output>
    where
...
impl<const MAX_SIZE: usize, Output> Future for ErasedFuture<MAX_SIZE, Output> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        unsafe { (self.poll)(Pin::new_unchecked(&mut self.get_unchecked_mut().erased), cx) }
    }
}
