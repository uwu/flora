...
pub trait OpMappingContextLifetime<'s, 'i> {
    ...
    fn map_mapping_error(context: &mut Self::Context, err: Self::MappingError) -> Self::Result;
}
...
pub trait OpMappingContext: for<'s, 'i> OpMappingContextLifetime<'s, 'i> + 'static {
...
pub type V8RetValMapper<R> = for<'r, 'i> fn(
    &mut v8::PinScope<'r, 'i>,
    R,
...
impl<'s, 'i> OpMappingContextLifetime<'s, 'i> for V8OpMappingContext {
    ...
    #[inline(always)]
    fn map_error(scope: &mut v8::PinScope<'s, 'i>, err: JsErrorBox) -> Self::Result {
...
impl<C: OpMappingContext> PendingOp<C> {
    ...
    #[inline(always)]
    pub fn ok<R: 'static>(info: PendingOpInfo, rv_map: C::MappingFn<R>, r: R) -> Self {
...
pub struct PendingOpMappingInfo<C: OpMappingContext, R: 'static, const FALLIBLE: bool>(
    pub PendingOpInfo,
    pub C::MappingFn<R>,
);
...
impl<C: OpMappingContext, R: 'static> FutureContextMapper<PendingOp<C>, PendingOpInfo, R>
...
impl<C: OpMappingContext> OpResult<C> {
    ...
    pub fn unwrap<'a, 'i>(
        ...
        let (success, res) = match self {
            ...
            Self::Value(f) => (true, (f.map_fn)(PhantomData, context, f.rv_map, f.value)),
            Self::ValueLarge(f) => (true, f.unwrap(context)),
        };
        match (success, res) {
            (true, Ok(x)) => Ok(x),
            (false, Ok(x)) => Err(x),
            (_, Err(err)) => Err(<C as OpMappingContextLifetime<'a, 'i>>::map_mapping_error(
                context, err,
            )),
        }
    }
