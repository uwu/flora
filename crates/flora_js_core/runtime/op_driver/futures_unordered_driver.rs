...
pub struct FuturesUnorderedDriver<C: OpMappingContext + 'static = V8OpMappingContext> {
    len: Cell<usize>,
    task: Cell<MaybeTask>,
    task_set: Cell<bool>,
    queue: SubmissionQueue<FuturesUnordered<FutureAllocation<PendingOp<C>, PendingOpInfo>>>,
...
impl<C: OpMappingContext> OpDriver<C> for FuturesUnorderedDriver<C> {
    ...
    >(
        ...
        {
            let info = PendingOpMappingInfo::<_, _, true>(PendingOpInfo(promise_id, op_id), rv_map);
    ...
    fn submit_op_infallible<R: 'static, const LAZY: bool, const DEFERRED: bool>(
        ...
        {
            let info =
                PendingOpMappingInfo::<_, _, false>(PendingOpInfo(promise_id, op_id), rv_map);
            let mut pinned = self.arena.allocate(info, op);
