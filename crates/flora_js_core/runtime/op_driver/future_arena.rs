...
#[pin_project]
struct DynFutureInfo<T: 'static, C: 'static, M: FutureContextMapper<T, C, F::Output>, F: Future> {
    /// The future metadata
    #[pin]
    context: M,
...
impl<T, C, M: FutureContextMapper<T, C, F::Output>, F: Future> ContextFuture<T, C>
...
impl<T, C> Future for FutureAllocation<T, C> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        unsafe {
            match self.get_mut() {
                Self::Arena(a) => Pin::new_unchecked(a.ptr.assume_init().as_mut()).poll(cx),
...
impl<T, C, M: FutureContextMapper<T, C, F::Output>, F: Future> TypedFutureAllocation<T, C, M, F> {
    #[inline(always)]
        ...
        if std::mem::size_of::<DynFutureInfo<T, C, M, F>>() <= MAX_ARENA_FUTURE_SIZE {
            unsafe {
                if let Some(reservation) = self.arena.reserve_space() {
                    ...
                    let ptr = TypeErased::raw_ptr::<DynFutureInfo<T, C, M, F>>(alloc.data.get());
...
mod tests {
    ...
    fn test_mapping() {
        ...
        let Poll::Ready(v) = f.poll_unpin(&mut Context::from_waker(Waker::noop())) else {
            panic!();
        };
        assert_eq!(v, 1);
