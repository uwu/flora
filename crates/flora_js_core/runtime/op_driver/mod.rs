...
pub(crate) trait OpDriver<C: OpMappingContext = V8OpMappingContext>: Default {
    ...
    fn submit_op_infallible_scheduling<R: 'static>(
        ...
        match scheduling {
            OpScheduling::Eager => {
                self.submit_op_infallible::<R, false, false>(op_id, promise_id, op, rv_map)
            }
            OpScheduling::Lazy => {
                self.submit_op_infallible::<R, true, false>(op_id, promise_id, op, rv_map)
            }
            OpScheduling::Deferred => {
                self.submit_op_infallible::<R, false, true>(op_id, promise_id, op, rv_map)
            }
        }
    }
    ...
    fn submit_op_fallible_scheduling<R: 'static, E: JsErrorClass + 'static>(
        ...
        match scheduling {
            OpScheduling::Eager => {
                self.submit_op_fallible::<R, E, false, false>(op_id, promise_id, op, rv_map)
            }
            OpScheduling::Lazy => {
                self.submit_op_fallible::<R, E, true, false>(op_id, promise_id, op, rv_map)
            }
            OpScheduling::Deferred => {
                self.submit_op_fallible::<R, E, false, true>(op_id, promise_id, op, rv_map)
            }
        }
    }
    ...
    fn poll_ready(&self, cx: &mut Context) -> Poll<(PromiseId, OpId, OpResult<C>)>;
...
mod tests {
    ...
    impl<'s, 'i> OpMappingContextLifetime<'s, 'i> for TestMappingContext {
        ...
        fn map_error(_context: &mut Self::Context, err: JsErrorBox) -> Self::Result {
            format!("{err:?}")
        }
    ...
    fn submit_task(
        ...
        assert_eq!(
            None,
            driver.submit_op_infallible_scheduling(scheduling, 1234, id as _, op, |r| {
                Ok(format!("{r}"))
            })
        );
    }
    ...
    fn submit_task_eager_ready(
        ...
        assert_eq!(
            Some(result),
            driver.submit_op_infallible_scheduling(OpScheduling::Eager, 1234, id as _, op, |r| {
                Ok(format!("{r}"))
            })
        );
    }
