...
static ISOLATE_ENTRIES: std::sync::LazyLock<Mutex<HashMap<usize, IsolateEntry>>> =
...
fn v8_init(v8_platform: Option<v8::SharedRef<v8::Platform>>, snapshot: bool, expose_natives: bool) {
    ...
    let flags = match (snapshot, expose_natives) {
        ...
        (true, true) => {
            format!("{base_flags} {snapshot_flags} {lazy_flags} {expose_natives_flags}")
        }
    };
    v8::V8::set_flags_from_string(&flags);

    let v8_platform = v8_platform.unwrap_or_else(|| {
        let unprotected = cfg!(any(test, feature = "unsafe_use_unprotected_platform"));
        ...
        v8::new_custom_platform(thread_pool_size, false, unprotected, DenoPlatformImpl)
            .make_shared()
    });
...
pub fn create_isolate(
    ...
    let mut isolate = if will_snapshot {
        snapshot::create_snapshot_creator(external_refs, maybe_startup_snapshot, params)
    } else {
        ...
        if cfg!(windows) && has_snapshot && FIRST_SNAPSHOT_INIT.load(Ordering::SeqCst) {
    ...
    isolate.set_prepare_stack_trace_callback(crate::error::prepare_stack_trace_callback);
    ...
    isolate.set_wasm_async_resolve_promise_callback(bindings::wasm_async_resolve_promise_callback);
    isolate.set_wasm_streaming_callback(crate::ops_builtin_v8::wasm_streaming_callback);
