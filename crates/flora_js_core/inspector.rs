...
fn extend_capped(target: &mut Vec<u8>, bytes: &[u8]) {
    let remaining = NETWORK_BUFFER_MAX_BYTES_PER_REQUEST.saturating_sub(target.len());
...
fn make_cdp_ok_response(id: &serde_json::Value, result: serde_json::Value) -> String {
    json!({
      "id": id,
      "result": result,
    })
    ...
    match method {
        ...
        | "Network.getRequestPostData" => {
            ...
            let result = match method {
                "Network.getResponseBody" => build_get_response_body(&mut buf, request_id),
                "Network.streamResourceContent" => {
...
pub fn create_worker_inspector_session_pair(
    ...
    let (worker_to_main_tx, worker_to_main_rx) = mpsc::unbounded::<InspectorMsg>();
...
impl JsRuntimeInspectorState {
    ...
    pub fn poll_sessions(&self, mut invoker_cx: Option<&mut Context>) -> Result<Poll<()>, ()> {
        ...
        loop {
            loop {
                ...
                if let Poll::Ready(Some(session_proxy)) = sessions.session_rx.poll_next_unpin(cx) {
...
impl JsRuntimeInspector {
    ...
    pub fn new(
        ...
        *state.sessions.borrow_mut() = SessionContainer::new(
        ...
        v8_inspector.context_created(context, Self::CONTEXT_GROUP_ID, context_name, aux_data_view);
    ...
    pub fn wait_for_session(&self) {
        loop {
            if let Some(_session) = self.state.sessions.borrow_mut().local.values().next() {
    ...
    pub fn wait_for_page_wait_for_debugger(&self) {
        const PAGE_WAIT_GRACE_PERIOD: std::time::Duration = std::time::Duration::from_millis(500);
    ...
    pub fn wait_for_debugger_enabled_for_worker_message(&self) {
        ...
        let deadline = std::time::Instant::now() + WORKER_MESSAGE_DEBUGGER_GRACE_PERIOD;
        loop {
    ...
    /// into streaming mode.
    pub fn capture_network_event(&self, event_name: &str, params: &serde_json::Value) -> bool {
...
impl SessionContainer {
    ...
    fn sessions_state(&self) -> SessionsState {
        SessionsState {
            ...
            has_nonblocking: self
                .local
                .values()
                .any(|s| matches!(s.state.kind, InspectorSessionKind::NonBlocking { .. })),
    ...
    pub fn dispatch_message_from_frontend(&mut self, session_id: i32, message: String) {
    ...
    fn register_worker_session(&mut self, local_session_id: i32, worker_url: String) -> u32 {
    ...
    pub fn register_worker_channels(
        ...
        for target_session in self.target_sessions.values() {
            if target_session.local_session_id == local_session_id {
                *target_session.main_worker_channels.borrow_mut() = Some(MainWorkerChannels {
                    main_to_worker_tx,
                    worker_to_main_rx,
                });
                return true;
    ...
    fn target_session_by_target_id(&self, target_id: &str) -> Option<Rc<TargetSession>> {
        self.target_sessions
            .values()
...
async fn pump_inspector_session_messages(session: Rc<InspectorSession>, session_id: i32) -> i32 {
    ...
    while let Some(msg) = rx.next().await {
        ...
        match method {
            ...
            "Target.setAutoAttach" => {
                ...
                let wait_for_debugger_on_start = get_bool_param(&params, "waitForDebuggerOnStart");
                let send = session.state.send.clone();
                let sessions = session.state.sessions.clone();
...
impl LocalInspectorSession {
    ...
    pub fn post_message<T: serde::Serialize>(&mut self, id: i32, method: &str, params: Option<T>) {
        let message = json!({
            "id": id,
            "method": method,
            "params": params,
