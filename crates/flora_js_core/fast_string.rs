...
impl FastString {
    ...
    pub fn v8_string<'a, 'i>(
        ...
        match self.inner {
            ...
            FastStringInner::StaticConst(s) => {
                v8::String::new_from_onebyte_const(scope, s.s).ok_or(FastStringV8AllocationError)
            }
            _ => v8::String::new_from_utf8(scope, self.as_bytes(), NewStringType::Normal)
                .ok_or(FastStringV8AllocationError),
        }
    }
    ...
    pub fn truncate(&mut self, index: usize) {
        match &mut self.inner {
            FastStringInner::Static(b) => self.inner = FastStringInner::Static(&b[..index]),
...
impl<'de> serde::Deserialize<'de> for FastString {
    ...
    where
        D: Deserializer<'de>,
    {
        DeserializeProxy::<'de>::deserialize(deserializer).map(|v| v.to_owned().into())
    }
}
