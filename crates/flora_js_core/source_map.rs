...
impl SourceMapper {
    ...
    pub(crate) fn take_ext_source_maps(&mut self) -> HashMap<ModuleName, SourceMapData> {
        std::mem::take(&mut self.ext_source_maps)
    }

    /// Add a source map extracted from V8 for a module.
    pub(crate) fn add_source_map(&mut self, module_name: ModuleName, source_map: SourceMap) {
        self.maps
            .insert(module_name.to_string(), Some(Arc::new(source_map)));
    }

    pub(crate) fn add_source_map_url(&mut self, module_name: ModuleName, source_map_url: String) {
    ...
    pub fn apply_source_map(
        ...
        let maybe_source_map = self.maps.entry(file_name.to_owned()).or_insert_with(|| {
            None
                ...
                .or_else(|| {
                    ...
                    let source_map_data = self.loader.load_external_source_map(source_map_url)?;
    ...
    pub fn get_source_line(&mut self, file_name: &str, line_number: i64) -> Option<String> {
...
mod tests {
    ...
    fn test_source_mapper() {
      ...
      SourceMapLoaderContent {
        ...
        // Non-existent file
        let application = source_mapper.apply_source_map("file:///doesnt_exist.js", 1, 1);
    ...
    fn test_source_map_relative_path_nonexistent_file() {
      ...
      SourceMapLoaderContent {
        ...
        let application = source_mapper.apply_source_map("file:///project/dist/bundle.js", 1, 1);
        assert_eq!(
            application,
            SourceMapApplication::LineAndColumn {
                line_number: 1,
