...
pub unsafe fn uv_read_start(
    ...
    unsafe {
        if (*stream).r#type == uv_handle_type::UV_TTY {
            return super::tty::read_start_tty(stream as *mut uv_tty_t, alloc_cb, read_cb);
        }
...
pub unsafe fn uv_read_stop(stream: *mut uv_stream_t) -> c_int {
    unsafe {
        ...
        if (*stream).r#type == uv_handle_type::UV_NAMED_PIPE {
            return super::pipe::read_stop_pipe(stream as *mut super::pipe::uv_pipe_t);
        }
        // SAFETY: Caller guarantees stream is a valid, initialized uv_tcp_t.
        let tcp = stream as *mut uv_tcp_t;
        let tcp_ref = &mut *tcp;
...
/// `stream` must be a valid pointer to an initialized stream handle.
#[cfg(unix)]
pub unsafe fn uv_stream_set_blocking(stream: *mut uv_stream_t, blocking: c_int) -> c_int {
    unsafe {
...
/// `stream` must be a valid pointer to an initialized stream handle.
#[cfg(windows)]
pub unsafe fn uv_stream_set_blocking(_stream: *mut uv_stream_t, _blocking: c_int) -> c_int {
...
/// this call.
pub unsafe fn uv_try_writev(handle: *mut uv_stream_t, bufs: &[std::io::IoSlice<'_>]) -> i32 {
    ...
    unsafe {
        ...
        if handle_type == uv_handle_type::UV_NAMED_PIPE {
            let mut total = 0i32;
            for buf in bufs {
                let rc = super::pipe::try_write_pipe(handle as *mut super::pipe::uv_pipe_t, buf);
                if rc < 0 {
                    if total > 0 {
                        return total;
                    }
    ...
    if tcp_ref.internal_connect.is_some() || !tcp_ref.internal_write_queue.is_empty() {
        return UV_EAGAIN;
    }
...
pub unsafe fn uv_try_write(handle: *mut uv_stream_t, data: &[u8]) -> i32 {
    ...
    unsafe {
        ...
        if handle_type == uv_handle_type::UV_NAMED_PIPE {
            return super::pipe::try_write_pipe(handle as *mut super::pipe::uv_pipe_t, data);
        }
    }
    // SAFETY: Caller guarantees handle is a valid, initialized uv_tcp_t.
    let tcp_ref = unsafe { &mut *(handle as *mut uv_tcp_t) };
...
pub unsafe fn uv_write(
    ...
    unsafe {
        ...
        if (*handle).r#type == uv_handle_type::UV_NAMED_PIPE {
            return write_pipe(req, handle as *mut super::pipe::uv_pipe_t, bufs, nbufs, cb);
        }
...
pub unsafe fn uv_write_owned(
    ...
    unsafe {
        match (*handle).r#type {
            uv_handle_type::UV_TCP => uv_write_owned_tcp(req, handle as *mut uv_tcp_t, data, cb),
...
pub(crate) unsafe fn complete_shutdown(tcp: *mut uv_tcp_t, cx: &mut std::task::Context<'_>) {
    use std::pin::Pin;
    ...
    let status = if let Some(ref mut stream) = unsafe { &mut *tcp }.internal_stream {
...
unsafe fn write_pipe(
    ...
    unsafe {
            ...
            && (*pipe).internal_fd.is_some()
        {
            ...
            if let Ok(afd) = tokio::io::unix::AsyncFd::new(super::pipe::RawFdWrapper(fd)) {
