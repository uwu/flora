...
pub struct uv_pipe_t {
    ...
    #[cfg(windows)]
    pub(crate) internal_win_client: Option<tokio::net::windows::named_pipe::NamedPipeClient>,
    /// Pending `NamedPipeServer::connect()` future. When ready, a client
    /// has connected and the connection callback should fire.
    #[cfg(windows)]
    #[allow(clippy::type_complexity, reason = "pinned boxed async connect future")]
    pub(crate) internal_win_connect_fut:
    ...
    pub(crate) internal_win_connect_retry: Option<
        tokio::task::JoinHandle<std::io::Result<tokio::net::windows::named_pipe::NamedPipeClient>>,
    >,
    ...
    pub(crate) ipc: bool,
    pub(crate) internal_waker: Option<std::sync::Arc<crate::uv_compat::waker::PipeHandleWaker>>,
}
...
pub(crate) struct PipeConnectPending {
    ...
    pub future: std::pin::Pin<
        Box<dyn std::future::Future<Output = std::io::Result<tokio::net::UnixStream>>>,
    >,
}
...
/// # Safety
/// `pipe` must be a valid pointer to an initialized `uv_pipe_t`.
pub unsafe fn uv_pipe_set_pending_instances(pipe: *mut uv_pipe_t, instances: i32) {
    unsafe {
...
pub unsafe fn uv_pipe_init(loop_: *mut uv_loop_t, pipe: *mut uv_pipe_t, ipc: c_int) -> c_int {
    unsafe {
        *pipe = new_pipe(ipc != 0);
        (*pipe).loop_ = loop_;
        ...
        (*pipe).internal_waker = Some(crate::uv_compat::waker::PipeHandleWaker::new(
            pipe as usize,
            shared,
        ));
    }
        ...
        if GetFileType(dup) == FILE_TYPE_PIPE {
            ...
            if info_ok != 0 {
                ...
                let primary_ok = if is_server {
                    match tokio::net::windows::named_pipe::NamedPipeServer::from_raw_handle(raw) {
                        Ok(server) => {
                    ...
                    match tokio::net::windows::named_pipe::NamedPipeClient::from_raw_handle(raw) {
                        Ok(client) => {
                ...
                (*pipe).internal_handle = Some(raw_handle_dup as std::os::windows::io::RawHandle);
                return 0;
            }
        }
...
#[cfg(unix)]
fn make_sockaddr_un(path: &str) -> Result<(libc::sockaddr_un, libc::socklen_t), c_int> {
...
#[cfg(unix)]
pub unsafe fn uv_pipe_accept(server: *mut uv_pipe_t, client: *mut uv_pipe_t) -> c_int {
    unsafe {
...
pub unsafe fn uv_pipe_connect(
    ...
    let future = Box::pin(async move {
        if let Some(fd) = bound_fd {
            ...
            unsafe {
                let (addr, addr_len) = make_sockaddr_un(path.as_str()).map_err(|_| {
                ...
                if ret != 0 {
                    ...
                    let _guard = async_fd.writable().await.map_err(std::io::Error::other)?;
        ...
        match opts.open(path) {
            Ok(client) => {
                ...
                if let Some(cb) = cb {
                    ...
                    (*pipe).internal_connect = Some(PipeConnectPending { req, cb: Some(cb) });
                }
            ...
            Err(e) if e.raw_os_error() == Some(231) => {
                ...
                let join =
                    tokio::task::spawn_blocking(move || wait_named_pipe_and_open(&path_owned));
...
pub(crate) fn wait_named_pipe_and_open(
    ...
    loop {
        ...
        let ok = unsafe { windows_sys::Win32::System::Pipes::WaitNamedPipeW(wide.as_ptr(), 30000) };
        if ok == 0 {
...
/// # Safety
/// `pipe` must be a valid pointer to an initialized `uv_pipe_t`.
pub(crate) unsafe fn try_write_pipe(pipe: *mut uv_pipe_t, data: &[u8]) -> c_int {
    unsafe {
        // Queued writes must complete in order; don't front-run them.
        if !(*pipe).internal_write_queue.is_empty() || (*pipe).internal_connect.is_some() {
        ...
        #[cfg(unix)]
        {
            if let Some(ref stream) = (*pipe).internal_stream {
                return match stream.try_write(data) {
                    Ok(n) => n as c_int,
                    Err(ref e) if e.kind() == std::io::ErrorKind::WouldBlock => super::UV_EAGAIN,
                    Err(ref e) => io_error_to_uv(e),
                };
            }
            if let Some(fd) = (*pipe).internal_fd {
                let n = libc::write(fd, data.as_ptr() as *const std::ffi::c_void, data.len());
                return if n >= 0 {
                    n as c_int
                } else {
...
#[cfg(windows)]
pub(crate) unsafe fn poll_pipe_handle(pipe_ptr: *mut uv_pipe_t, cx: &mut Context<'_>) -> bool {
    use std::task::Poll;
    ...
    unsafe {
        ...
        if (*pipe_ptr).internal_reading {
                        ...
                        match result {
                            ...
                            Ok(Ok((data, n))) => {
                                ...
                                if !buf.base.is_null() && buf.len > 0 {
                                    ...
                                    read_cb(pipe_ptr as *mut uv_stream_t, copy_len as isize, &buf);
                                }
                                if (*pipe_ptr).internal_reading {
                                    maybe_spawn_win_read(pipe_ptr);
                                }
            ...
            if has_named_pipe {
                ...
                if let (Some(alloc_cb), Some(read_cb)) = (alloc_cb, read_cb) {
                    ...
                    alloc_cb(pipe_ptr as *mut uv_handle_t, 65536, &mut buf);
                    if !buf.base.is_null() && buf.len > 0 {
                        let slice = std::slice::from_raw_parts_mut(buf.base.cast::<u8>(), buf.len);
                        let read_result = if let Some(ref server) = (*pipe_ptr).internal_win_server
                        {
        ...
        if (*pipe_ptr).internal_write_queue.is_empty() && (*pipe_ptr).internal_shutdown.is_some() {
...
pub(crate) unsafe fn poll_pipe_handle(pipe_ptr: *mut uv_pipe_t, cx: &mut Context<'_>) -> bool {
    ...
    unsafe {
        ...
        let connect_result = if let Some(ref mut pending) = (*pipe_ptr).internal_connect {
        ...
        while let Some(pw) = (*pipe_ptr).internal_write_queue.front() {
            ...
            let write_result = if let Some(ref afd) = (*pipe_ptr).internal_async_fd {
                ...
                let n = libc::write(fd, remaining.as_ptr() as *const c_void, remaining.len());
                if n >= 0 {
                    Ok(n as usize)
                } else {
        ...
        let can_read = (*pipe_ptr).internal_reading
            && ((*pipe_ptr).internal_async_fd.is_some() || (*pipe_ptr).internal_stream.is_some());
        if can_read {
            let alloc_cb = (*pipe_ptr).internal_alloc_cb;
            let read_cb = (*pipe_ptr).internal_read_cb;
