...
pub struct uv_tty_t {
    ...
    pub(crate) internal_waker: Option<std::sync::Arc<crate::uv_compat::waker::TtyHandleWaker>>,
}

#[cfg(windows)]
type LineReadResult = std::sync::Arc<std::sync::Mutex<Option<Result<Vec<u8>, i32>>>>;
...
mod global_termios {
    ...
    static ORIG_TERMIOS: SyncTermios = SyncTermios(UnsafeCell::new(MaybeUninit::uninit()));
...
pub(crate) mod win_console {
    ...
    pub unsafe fn safe_get_osfhandle(fd: c_int) -> isize {
        unsafe {
            let prev =
                _set_thread_local_invalid_parameter_handler(Some(noop_invalid_parameter_handler));
...
mod global_console {
    ...
    static CONIN_HANDLE: std::sync::atomic::AtomicUsize = std::sync::atomic::AtomicUsize::new(0);
    // Whether mode was ever changed and needs reset.
    static NEEDS_RESET: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
...
#[cfg(unix)]
unsafe fn tty_try_write(tty: *const uv_tty_t, data: &[u8]) -> std::io::Result<usize> {
    let fd = unsafe { (*tty).internal_fd };
    let ret = unsafe { libc::write(fd, data.as_ptr() as *const c_void, data.len()) };
    if ret < 0 {
...
#[cfg(unix)]
unsafe fn tty_try_read(tty: *const uv_tty_t, buf: &mut [u8]) -> std::io::Result<usize> {
    let fd = unsafe { (*tty).internal_fd };
...
#[cfg(windows)]
fn get_vt100_fn_key(code: u16, shift: bool, ctrl: bool) -> Option<&'static [u8]> {
    use win_console::*;
...
/// available.
#[cfg(windows)]
unsafe fn tty_try_read_raw(tty: *mut uv_tty_t, buf: &mut [u8]) -> std::io::Result<usize> {
    let handle = unsafe { (*tty).internal_handle };
    ...
    loop {
        ...
        unsafe {
            while (*tty).internal_last_key_offset < (*tty).internal_last_key_len {
                ...
                buf[buf_used] = (*tty).internal_last_key[(*tty).internal_last_key_offset as usize];
        ...
        if unsafe { win_console::GetNumberOfConsoleInputEvents(handle, &mut num_events) } == 0
            || num_events == 0
        {
            break;
        }
        ...
        if unsafe { win_console::ReadConsoleInputW(handle, &mut record, 1, &mut records_read) } == 0
        {
        ...
        if kev.bKeyDown == 0 && (kev.wVirtualKeyCode != win_console::VK_MENU || kev.uChar == 0) {
            continue;
        }
        ...
        if kev.uChar != 0 {
            ...
            let decoded = if high_surrogate != 0 {
                ...
                char::decode_utf16([high_surrogate, unicode_char].iter().copied()).next()
            } else {
            ...
            unsafe {
                (&mut (*tty).internal_last_key)[..total_len].copy_from_slice(&key_buf[..total_len]);
            ...
            unsafe {
                ...
                (&mut (*tty).internal_last_key)[prefix_len..total_len].copy_from_slice(vt100);
                (*tty).internal_last_key_len = total_len as u8;
                (*tty).internal_last_key_offset = 0;
                (*tty).internal_last_repeat_count = kev.wRepeatCount.saturating_sub(1);
            }
...
pub unsafe fn uv_tty_init(
    ...
    unsafe {
        ...
        #[cfg(unix)]
        {
            ...
            if handle_type == uv_handle_type::UV_TTY && tty_is_slave(fd) {
                ...
                if tty_path(fd, &mut path) {
                    let new_fd = open_cloexec(path.as_ptr().cast(), mode | libc::O_NOCTTY);
                    if new_fd >= 0 {
                        actual_fd = new_fd;
                        reopened = true;
                    }
            ...
            if cur_flags & libc::O_NONBLOCK == 0
                && libc::fcntl(actual_fd, libc::F_SETFL, cur_flags | libc::O_NONBLOCK) == -1
            {
                if reopened {
        ...
        let (win_handle, win_readable, win_saved_mode, win_handle_owned) = {
            ...
            let mut dummy: u32 = 0;
            let readable = win_console::GetNumberOfConsoleInputEvents(handle, &mut dummy) != 0;
            ...
            if !readable {
                let mut info: win_console::CONSOLE_SCREEN_BUFFER_INFO = std::mem::zeroed();
...
pub unsafe fn uv_tty_set_mode(tty: *mut uv_tty_t, mode: uv_tty_mode_t) -> c_int {
    unsafe {
        #[cfg(unix)]
        {
            use uv_tty_mode_t::*;
            ...
            if (*tty).mode == UV_TTY_MODE_NORMAL && effective_mode != UV_TTY_MODE_NORMAL {
            ...
            match effective_mode {
                ...
                UV_TTY_MODE_RAW => {
                    ...
                    tmp.c_lflag &= !(libc::ECHO | libc::ICANON | libc::IEXTEN | libc::ISIG);
...
/// as `uv_stream_t`).
pub(crate) unsafe fn try_write_tty(handle: *mut uv_stream_t, data: &[u8]) -> i32 {
...
#[cfg(windows)]
unsafe extern "system" fn win_tty_wait_callback(context: *mut c_void, _timer_or_wait_fired: u8) {
...
unsafe fn tty_register_wait(tty: *mut uv_tty_t, waker: &Waker) {
    unsafe {
        ...
        if (*tty).internal_wait_waker.is_none() {
            (*tty).internal_wait_waker = Some(Box::new(std::sync::Mutex::new(Some(waker.clone()))));
...
unsafe fn tty_unregister_wait(tty: *mut uv_tty_t) {
    unsafe {
        if !(*tty).internal_wait_handle.is_null() {
            ...
            win_console::UnregisterWaitEx((*tty).internal_wait_handle, -1isize as *mut c_void);
...
pub(crate) unsafe fn poll_tty_handle(tty_ptr: *mut uv_tty_t, cx: &mut Context<'_>) -> bool {
                ...
                #[cfg(not(unix))]
                {
                    if (*tty_ptr).internal_readable && (*tty_ptr).internal_reading {
                        if is_raw_tty_mode((*tty_ptr).mode) {
                            ...
                            if (*tty_ptr).internal_reading && !(*tty_ptr).internal_line_read_pending
                            {
    ...
    unsafe {
        if !(*tty_ptr).internal_write_queue.is_empty() {
            ...
            loop {
                ...
                let pre_status = (*tty_ptr).internal_write_queue.front().unwrap().status;
    ...
    unsafe {
        if (*tty_ptr).internal_write_queue.is_empty() && (*tty_ptr).internal_shutdown.is_some() {
    ...
    unsafe {
        if drained_select_signal
            && let Some(TtyReactor::SelectFallback(s)) = (*tty_ptr).internal_reactor.as_ref()
...
fn kqueue_rejects_fd(fd: RawFd) -> bool {
    unsafe {
        ...
        ret > 0 && (events.flags & libc::EV_ERROR) != 0 && events.data as c_int == libc::EINVAL
    }
}
...
fn setup_select_fallback(fd: RawFd) -> Result<SelectFallbackState, c_int> {
    ...
    for &sock_fd in &[fake_fd, int_fd] {
        ...
        if flags == -1
            || unsafe { libc::fcntl(sock_fd, libc::F_SETFL, flags | libc::O_NONBLOCK) } == -1
        {
            unsafe {
                libc::close(fake_fd);
                libc::close(int_fd);
    ...
    let thread = match std::thread::Builder::new()
        ...
        .spawn(move || {
            select_thread_main(fd, int_fd, max_fd, close_flag2, interest2, async_sem2);
        }) {
        Ok(t) => t,
        Err(_) => {
...
unsafe fn update_select_interest(tty: *mut uv_tty_t) {
    ...
    unsafe {
        if let Some(TtyReactor::SelectFallback(s)) = (*tty).internal_reactor.as_ref() {
            let mut flags: u8 = 0;
            if (*tty).internal_reading {
                flags |= SELECT_INTEREST_READ;
            }
            if !(*tty).internal_write_queue.is_empty() || (*tty).internal_shutdown.is_some() {
...
unsafe fn collect_bufs(bufs: *const uv_buf_t, nbufs: u32) -> Vec<u8> {
    unsafe {
        ...
        for i in 0..nbufs as usize {
            let buf = &*bufs.add(i);
            if !buf.base.is_null() && buf.len > 0 {
                data.extend_from_slice(std::slice::from_raw_parts(buf.base as *const u8, buf.len));
            }
        }
        data
    }
