...
pub struct uv_tcp_t {
    ...
    pub(crate) internal_waker: Option<std::sync::Arc<crate::uv_compat::waker::TcpHandleWaker>>,
}
...
pub(crate) struct ConnectPending {
    pub(crate) future: Pin<Box<dyn Future<Output = std::io::Result<tokio::net::TcpStream>>>>,
...
#[allow(
    deprecated,
    reason = "libuv close-reset semantics require SO_LINGER before drop"
)]
...
#[cfg(unix)]
unsafe fn apply_keepalive_fd(fd: std::os::unix::io::RawFd, enable: bool, delay: c_uint) -> c_int {
...
#[cfg(windows)]
unsafe fn apply_keepalive_socket(sock: usize, enable: bool, _delay: c_uint) -> c_int {
    unsafe extern "system" {
        fn setsockopt(
            s: usize,
            level: c_int,
...
#[cfg_attr(feature = "uv_compat_export", unsafe(no_mangle))]
pub unsafe extern "C" fn uv_tcp_simultaneous_accepts(_tcp: *mut uv_tcp_t, _enable: c_int) -> c_int {
    0 // no-op
}
...
pub unsafe fn uv_accept(server: *mut uv_stream_t, client: *mut uv_stream_t) -> c_int {
...
pub(crate) unsafe fn poll_tcp_handle(tcp_ptr: *mut uv_tcp_t, cx: &mut Context<'_>) -> bool {
        ...
        if (*tcp_ptr).internal_reading && (*tcp_ptr).internal_stream.is_some() {
            ...
            if let (Some(alloc_cb), Some(read_cb)) = (alloc_cb, read_cb) {
                ...
                loop {
                    ...
                    if !(*tcp_ptr).internal_reading || (*tcp_ptr).internal_stream.is_none() {
                        break;
                    }
                    ...
                    let read_result = (*tcp_ptr).internal_stream.as_ref().unwrap().try_read(slice);
                    match read_result {
                        Ok(0) => {
    ...
    let mut completed_writes: Vec<(*mut uv_write_t, Option<uv_write_cb>, c_int)> = Vec::new();
    unsafe {
        if !(*tcp_ptr).internal_write_queue.is_empty() && (*tcp_ptr).internal_stream.is_some() {
            if let Some(ref stream) = (*tcp_ptr).internal_stream {
                    ...
                    && let Poll::Ready(Ok(())) = stream.poll_read_ready(cx)
                {
                    ...
                    let n = {
                        ...
                        let mut probe = [0u8; 1];
                        libc::recv(fd, probe.as_mut_ptr() as *mut c_void, 1, libc::MSG_PEEK) as i32
                    };
                    #[cfg(windows)]
                    let n = {
                        use std::os::windows::io::AsRawSocket;
                        unsafe extern "system" {
                            fn recv(s: usize, buf: *mut c_void, len: c_int, flags: c_int) -> c_int;
                        }
    ...
    unsafe {
        if let Some(ref stream) = (*tcp_ptr).internal_stream {
            ...
            if (*tcp_ptr).internal_reading && matches!(stream.poll_read_ready(cx), Poll::Ready(_)) {
                needs_requeue = true;
            }
            if !(*tcp_ptr).internal_write_queue.is_empty()
