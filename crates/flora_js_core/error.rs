...
pub enum CoreErrorKind {
    ...
    #[class(generic)]
    #[error("Cannot evaluate module, because JavaScript execution has been terminated")]
    ExecutionTerminated,
    #[class(generic)]
...
impl CoreErrorKind {
    pub fn to_v8_error(&self, scope: &mut v8::PinScope) -> v8::Global<v8::Value> {
        ...
        let exception = js_class_and_message_to_exception(scope, &self.get_class(), &message);
...
pub fn throw_js_error_class(scope: &mut v8::PinScope, error: &dyn JsErrorClass) {
...
fn build_js_error_class_exception<'s, 'i>(
    ...
    let Ok(exception_obj) = TryInto::<v8::Local<v8::Object>>::try_into(exception) else {
        return exception;
    };
    ...
    exception_obj.create_data_property(scope, name_key.into(), class_value.into());
    ...
    for (key, value) in error.get_additional_properties() {
        ...
        let value = match value {
            PropertyValue::String(value) => v8::String::new(scope, &value).unwrap().into(),
    ...
    if !added_keys.is_empty() {
        ...
        let symbol_name = v8::String::new(scope, "errorAdditionalPropertyKeys").unwrap();
...
pub fn to_v8_error<'s, 'i>(
    ...
    let additional_properties = error
        ...
        .map(|(key, value)| {
            ...
            let value = match value {
                PropertyValue::String(value) => v8::String::new(tc_scope, &value).unwrap().into(),
    ...
    if !additional_properties.is_empty() {
        args.push(v8::Array::new_with_elements(tc_scope, &additional_properties).into());
    }
...
/// fmt_error.rs.
#[derive(
    Debug, PartialEq, Clone, serde::Deserialize, serde::Serialize, deno_ops::FromV8, deno_ops::ToV8,
)]
...
fn apply_source_map<'a>(
    ...
    match source_mapper.apply_source_map(&file_name, line_number as u32, column_number as u32) {
...
/// ```
///
fn parse_eval_origin(eval_origin: &str) -> Option<(&str, (&str, i64, i64), &str)> {
...
impl JsStackFrame {
      ...
      ($key: ident) => { call!($key : _) };
        ...
        let eval_origin = if is_wasm {
            ...
            call!(GET_EVAL_ORIGIN: Option<String>).and_then(|o| {
                let Some((before, (file, line, col), after)) = parse_eval_origin(&o) else {
                    return Some(o);
                };
                let state = JsRuntime::state_from(scope);
                let mut source_mapper = state.source_mapper.borrow_mut();
    ...
    pub fn from_v8_message<'s, 'i>(
        ...
        let state = JsRuntime::state_from(scope);
        let mut source_mapper = state.source_mapper.borrow_mut();
        let (file_name, line_num, col_num) = apply_source_map(&mut source_mapper, f.into(), l, c);
    ...
    v8::Local<'s, T>: TryFrom<v8::Local<'s, v8::Value>, Error: Debug>,
{
    let func = match get_property(scope, object, key)?.try_cast::<v8::Function>() {
        Ok(func) => func,
        Err(err) => {
            let message = format!("Callsite property '{key}' is not a function: {err}");
            let message = v8::String::new(scope, &message).unwrap();
            let exception = v8::Exception::type_error(scope, message);
            scope.throw_exception(exception);
            return None;
...
impl JsError {
    ...
    pub fn from_v8_exception<'s, 'i>(
        ...
        Box::new(
            Self::inner_from_v8_exception(scope, exception, Default::default(), 0, &mut state)
    ...
    fn inner_from_v8_exception<'s, 'i>(
        ...
        let js_format_exception_cb = exception_state.js_format_exception_cb.borrow().clone();
        ...
        if is_instance_of_error(scope, exception) {
            ...
            let e: NativeJsError = serde_v8::from_v8(scope, exception.into()).unwrap_or_default();
            ...
            let has_structured_frames = frames_v8.map(|a| a.length() > 0).unwrap_or(false);
            ...
            let additional_properties = exception.get(scope, additional_properties_key.into());
            ...
            let exception_message =
                exception_message.unwrap_or_else(|| v8_to_rust_string(&msg.get(scope), scope));
...
fn is_aggregate_error_inner<'s, 'i>(
    ...
    while let Some(prototype) = maybe_prototype {
        ...
        let prototype_name = match get_property(scope, prototype, v8_static_strings::CONSTRUCTOR) {
            Some(constructor) => {
...
fn abbrev_file_name<'a>(
    ...
    if url.scheme() == "data" {
        ...
        return Some(FileNameParts {
            working_dir_path: None,
            file_name: format!("{}:{},{}......{}", url.scheme(), head, start, end).into(),
        });
    }
...
fn format_eval_origin<'a>(eval_origin: &'a str, maybe_initial_cwd: Option<&Url>) -> Cow<'a, str> {
    let Some((before, (file, line, col), after)) = parse_eval_origin(eval_origin) else {
...
fn make_patched_callsite<'s, 'i>(
    ...
    let out_obj = v8::Object::with_prototype_and_properties(scope, prototype.into(), &[], &[]);
...
pub mod callsite_fns {
    ...
    impl<'s> SourceMappedCallsiteInfo<'s> {
        #[inline]
        fn file_name<'i>(&self, scope: &mut v8::PinScope<'s, 'i>) -> v8::Local<'s, v8::Value> {
            match self {
        ...
        #[inline]
        fn line_number<'i>(&self, scope: &mut v8::PinScope<'s, 'i>) -> v8::Local<'s, v8::Value> {
            match self {
        ...
        #[inline]
        fn column_number<'i>(&self, scope: &mut v8::PinScope<'s, 'i>) -> v8::Local<'s, v8::Value> {
            match self {
    ...
    fn maybe_apply_source_map<'s, 'i>(
        ...
        let convert::Number(line_number) = FromV8::from_v8(scope, line_number?).ok()?;
        let convert::Number(column_number) = FromV8::from_v8(scope, column_number?).ok()?;

        let state = JsRuntime::state_from(scope);
        let mut source_mapper = state.source_mapper.borrow_mut();
        let (mapped_file_name, mapped_line_number, mapped_column_number) = apply_source_map(
            &mut source_mapper,
            Cow::Owned(file_name),
            line_number,
            column_number,
    ...
    fn source_mapped_call_site_info<'s, 'i>(
        ...
        let file_name = call_method::<v8::Value>(scope, orig_callsite, super::GET_FILE_NAME, &[]);
        let line_number =
            call_method::<v8::Value>(scope, orig_callsite, super::GET_LINE_NUMBER, &[]);
        let column_number =
            call_method::<v8::Value>(scope, orig_callsite, super::GET_COLUMN_NUMBER, &[]);
                ...
                maybe_apply_source_map(scope, file_name, line_number, column_number)
        {
            ...
            let mapped_file_name = crate::FastString::from(mapped_file_name_trimmed)
                .v8_string(scope)
                .unwrap();
            let Ok(mapped_line_number) = convert::Number(mapped_line_number).to_v8(scope);
            let Ok(mapped_column_number) = convert::Number(mapped_column_number).to_v8(scope);
            ...
            let line_number = line_number.unwrap_or_else(|| v8::undefined(scope).into());
            let column_number = column_number.unwrap_or_else(|| v8::undefined(scope).into());
    ...
    make_callsite_fn!(get_script_name_or_source_url, GET_SCRIPT_NAME_OR_SOURCE_URL);
    ...
    fn to_string_inner<'s, 'i>(
        ...
        let orig_file_name = call_method::<v8::Value>(scope, orig, GET_FILE_NAME, &[])
            .and_then(|v| v.try_cast::<v8::String>().ok())?;
        ...
        let orig_line_number = call_method::<v8::Value>(scope, orig, GET_LINE_NUMBER, &[])
            .and_then(|v| v.try_cast::<v8::Number>().ok())?;
        let orig_column_number = call_method::<v8::Value>(scope, orig, GET_COLUMN_NUMBER, &[])
            .and_then(|v| v.try_cast::<v8::Number>().ok())?;
        ...
        let mapped_col_num = mapped.column_number(scope).cast::<v8::Number>().value() as i64;
    ...
    pub fn to_string<'s, 'i>(
        ...
        let Some(orig_to_string_v8) = call_method::<v8::String>(scope, orig, TO_STRING, &[]) else {
            return;
        };
...
fn prepare_stack_trace_inner<'s, 'i, const PATCH_CALLSITES: bool>(
    ...
    let global_error = get_property(scope, global, ERROR).and_then(|g| g.try_cast().ok());
    let prepare_fn = global_error.and_then(|g| {
        get_property(scope, g, PREPARE_STACK_TRACE).and_then(|f| f.try_cast::<v8::Function>().ok())
    });
...
pub fn format_stack_trace<'s, 'i>(
    ...
    for i in 0..callsites.length() {
        ...
        let Some(frame) = JsStackFrame::from_callsite_object(tc_scope, callsite) else {
            let exc = tc_scope
                .exception()
                .expect("JsStackFrame::from_callsite_object raised an exception");
            let message = match exc.to_string(tc_scope) {
...
pub trait ErrorFormat {
    fn fmt_element(element: ErrorElement, in_extension_code: bool, s: &str) -> Cow<'_, str>;
}

impl ErrorFormat for NoAnsiColors {
    fn fmt_element(_element: ErrorElement, _in_extension_code: bool, s: &str) -> Cow<'_, str> {
        s.into()
    }
}
...
pub fn format_location<F: ErrorFormat>(
    ...
    let in_extension_code = frame
        ...
        .unwrap_or(false);
    if frame.is_native {
        return F::fmt_element(NativeFrame, in_extension_code, "native").to_string();
    }
    let mut result = String::new();
    ...
    if !file_name.is_empty() {
        ...
        if let Some(working_dir_path) = &parts.working_dir_path {
            result += &F::fmt_element(WorkingDirPath, in_extension_code, working_dir_path);
        ...
        if frame.is_eval {
            ...
            let formatted_eval_origin = format_eval_origin(eval_origin, maybe_initial_cwd);
            result += &F::fmt_element(EvalOrigin, in_extension_code, &formatted_eval_origin);
    ...
    if frame.is_wasm {
        ...
        result += &F::fmt_element(PlainText, in_extension_code, ":");
        result += &F::fmt_element(LineNumber, in_extension_code, &line_number.to_string());
        if let Some(column_number) = frame.column_number {
            result += &F::fmt_element(PlainText, in_extension_code, ":");
            result += &F::fmt_element(ColumnNumber, in_extension_code, &column_number.to_string());
        }
    }
    result
}
...
pub fn format_frame<F: ErrorFormat>(
    ...
    if is_method_call {
        ...
        result += F::fmt_element(FunctionName, in_extension_code, &formatted_method).as_ref();
        ...
        if let Some(function_name) = &frame.function_name {
            ...
            result += F::fmt_element(Anonymous, in_extension_code, "<anonymous>").as_ref();
        }
    } else if let Some(function_name) = &frame.function_name {
        result += F::fmt_element(FunctionName, in_extension_code, function_name).as_ref();
    } else {
...
pub fn throw_error_one_byte_info(info: &v8::FunctionCallbackInfo, message: &str) {
...
pub fn throw_error_one_byte<'s, 'i>(scope: &mut v8::PinCallbackScope<'s, 'i>, message: &str) {
...
pub fn throw_invalid_this_error_one_byte_info(info: &v8::FunctionCallbackInfo, message: &str) {
    v8::callback_scope!(unsafe scope, info);
...
mod tests {
    ...
    fn test_is_aggregate_error_handles_throwing_accessors() {
        ...
        for source in [
            ...
            let exception: v8::Local<v8::Object> = JsRuntime::eval(scope, source).unwrap();
            let exception = exception.into();
    ...
    fn test_format_file_name() {
        ...
        .into_owned();
        let expected = "data:text/plain;base64,aaaaaaaaaaaaaaaaaaaa......aaaaaaa_%F0%9F%A6%95";
        ...
        let file_name = format_file_name("file:///%E6%9D%B1%E4%BA%AC/%F0%9F%A6%95.ts", None);
    ...
    fn test_parse_eval_origin() {
        ...
        for (input, expect) in cases {
            match expect {
                Some((expect_before, (expect_file, expect_line, expect_col), expect_after)) => {
