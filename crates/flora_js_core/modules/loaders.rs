...
impl ModuleLoader for NoopModuleLoader {
    ...
    fn load(
        ...
        ModuleLoadResponse::Sync(Err(JsErrorBox::generic("Module loading is not supported.")))
    }
}
...
impl ExtModuleLoader {
    ...
    pub fn finalize(&self) -> Result<(), CoreError> {
        ...
        if !unused_modules.is_empty() {
            return Err(CoreErrorKind::UnusedModules(
                unused_modules
                    .into_iter()
    ...
    pub fn new(
        ...
        Self {
            map: HashMap::from_iter(
                from.into_iter()
                    .map(|(url, code)| (url, code.into_module_code().into_cheap_copy().0)),
            ),
        }
    }
...
impl ModuleLoadEventCounts {
    pub fn new(resolve: usize, prepare: usize, finish: usize, load: usize) -> Self {
        Self {
            resolve,
            prepare,
            finish,
