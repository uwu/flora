...
impl ModuleSourceKey {
    pub fn from_reference(module_reference: &ModuleReference) -> Self {
        Self {
            ...
            typ: ModuleSourceType::Requested(module_reference.requested_module_type.clone()),
        }
    }

    pub fn from_loaded_source(loaded_source: &mut ModuleSource) -> Self {
        let name = if let Some(module_url_found) = loaded_source.cheap_copy_module_url_found() {
            module_url_found
        } else {
...
pub(crate) struct ModuleMapData {
    ...
    pub(crate) lazy_script_sources: Rc<RefCell<HashMap<ModuleName, ModuleCodeString>>>,
    pub(crate) residual_lazy_script_sources: &'static [(&'static str, &'static str)],
    ...
    /// not snapshotted.
    pub(crate) loaded_script_results: Rc<RefCell<HashMap<ModuleName, v8::Global<v8::Value>>>>,
    ...
    pub(crate) synthetic_esm_modules: Rc<RefCell<HashMap<ModuleName, ModuleName>>>,
...
impl ModuleMapData {
    ...
    pub(crate) fn register_under_type(
        ...
        self.by_name
            .insert(requested_module_type, name, SymbolicModule::Mod(module_id));
    }
    ...
    pub fn update_with_snapshotted_data(
        ...
        *self.known_lazy_esm.borrow_mut() = data.lazy_esm_specifiers.into_iter().collect();
