...
pub type ValidateImportAttributesCb =
    Box<dyn Fn(&mut v8::PinScope, &HashMap<String, String>, &ImportAttributesContext)>;
...
pub type EvalContextGetCodeCacheCb =
    Box<dyn Fn(&Url, &v8::String) -> Result<SourceCodeCacheInfo, deno_error::JsErrorBox>>;
...
impl ModuleType {
    pub fn to_v8<'s, 'i>(&self, scope: &mut v8::PinScope<'s, 'i>) -> v8::Local<'s, v8::Value> {
        match self {
...
impl ModuleSource {
    ...
    pub fn get_string_source(code: ModuleSourceCode) -> ModuleCodeString {
        match code {
            ...
            ModuleSourceCode::Bytes(bytes) => match String::from_utf8_lossy(bytes.as_bytes()) {
...
pub type ModuleSourceFuture = dyn Future<Output = Result<ModuleSource, ModuleLoaderError>>;
...
impl RequestedModuleType {
    pub fn to_v8<'s, 'i>(&self, scope: &mut v8::PinScope<'s, 'i>) -> v8::Local<'s, v8::Value> {
        match self {
            ...
            RequestedModuleType::Other(ty) => v8::String::new(scope, ty).unwrap().into(),
        }
    }
    ...
    pub fn into_error(
        ...
        match self {
            ModuleError::Exception(exception) => {
                ...
                CoreErrorKind::Js(exception_to_err(scope, exception, in_promise, clear_error))
                    .into_box()
            }
