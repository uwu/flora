...
fn debug_assert_residual_static_table_sorted(entries: &'static [(&'static str, &'static str)]) {
    debug_assert!(
...
pub(crate) struct ModuleMap {
    ...
    pending_tla_waiters: RefCell<HashMap<ModuleId, Vec<v8::Global<v8::PromiseResolver>>>>,
...
impl ModuleMap {
        ...
        for (handle, i) in data.handles_inverted.iter() {
            ...
            match module.get_status() {
                v8::ModuleStatus::Errored => {
                    return Err(CoreErrorKind::Js(JsError::from_v8_exception(
                        scope,
    ...
    pub(crate) fn register_under_type(
        ...
        self.data
            .borrow_mut()
            .register_under_type(name, requested_module_type, module_id);
    }
    ...
    pub(crate) fn new_module_with_pending(
        ...
        let module_id = match module_type {
            ModuleType::JavaScript => {
                ...
                self.new_module_from_js_source_with_pending(
                    scope,
                    main,
                    ModuleType::JavaScript,
                    module_url_found,
            ...
            ModuleType::Wasm => self.new_wasm_module(scope, module_url_found, code, dynamic)?,
            ...
            ModuleType::Bytes => {
                let ModuleSourceCode::Bytes(code) = code else {
                    return Err(ModuleError::Concrete(ModuleConcreteError::BytesNotBytes));
                };
    ...
    pub fn new_synthetic_module_from_exports_object<'s, 'i>(
        ...
        let mut export_names: Vec<v8::Local<v8::String>> = Vec::with_capacity(len as usize + 1);
        let mut export_values: Vec<v8::Local<v8::Value>> = Vec::with_capacity(len as usize + 1);
    ...
    pub(crate) fn new_module_from_js_source_with_pending(
        ...
        let _loading_internal_modules_guard = is_internal_module_specifier(name.as_str())
            ...
            && let Some(code_cache_info) = code_cache_info.take()
        {
            ...
            let fut =
                async move { (code_cache_info.ready_callback)(&code_cache).await }.boxed_local();
            self.code_cache_ready_futs.push(fut);
        }

        // Extract native source map URL from V8
        let unbound_module_script = module.get_unbound_module_script(tc_scope);
        let source_mapping_url_value = unbound_module_script.get_source_mapping_url(tc_scope);
        if !source_mapping_url_value.is_undefined() && !source_mapping_url_value.is_null() {
            ...
            if source_mapping_url.starts_with(DATA_PREFIX) {
                if let Ok(DecodedMap::Regular(sm)) = sourcemap::decode_data_url(&source_mapping_url)
                {
                    self.source_mapper
                        .borrow_mut()
                ...
                let resolved_url = if let Ok(module_url) = ModuleSpecifier::parse(name.as_str()) {
                    module_url
        ...
        for i in 0..module_requests.length() {
            ...
            // is thrown here
            {
                ...
                if let Some(validate_import_attributes_cb) = &state.validate_import_attributes_cb {
                    let location =
                        module.source_offset_to_location(module_request.get_source_offset());
    ...
    pub(crate) fn new_wasm_module_source(
        ...
        let Some(wasm_module) = v8::WasmModuleObject::compile(scope, code.as_bytes()) else {
            return Err(ModuleConcreteError::WasmCompile(loaded_key.name.to_string()).into());
        };
    ...
    pub fn resolve(
        ...
        if let Some(resolved_specifier) = self.maybe_resolve_internal_import(specifier, referrer) {
            return Ok(resolved_specifier);
        }
        let resolved_specifier = self.loader.borrow().resolve(specifier, referrer, kind)?;
    ...
    fn validate_ext_module_import(
        ...
        if (self.will_snapshot || self.loading_internal_modules.get()) && referrer == "." {
            return Ok(());
        }
    ...
    fn resolve_callback<'s, 'i>(
        ...
        if let Some(module) = self.try_resolve_synthetic_esm(scope, resolved_specifier.as_str()) {
            return Some(module);
        }
    ...
    pub(crate) fn load_dynamic_import(
        ...
        {
            match self.lazy_load_synthetic_esm_module(scope, module_specifier.as_str()) {
                Ok(module_ns) => {
                    let resolver = v8::Local::new(scope, &resolver_handle);
                    let module_ns_local = v8::Local::new(scope, module_ns);
                    resolver.resolve(scope, module_ns_local).unwrap();
    ...
    pub(crate) fn has_pending_dynamic_imports(&self) -> bool {
        self.preparing_dynamic_imports.is_pending() || self.pending_dynamic_imports.is_pending()
    }
    ...
    pub fn mod_evaluate<'s, 'i>(
        ...
        let receiver = receiver.map(|res| {
            res.map(|r| r.map_err(|r| CoreErrorKind::Js(r).into_box()))
    ...
    pub(crate) fn mod_evaluate_sync(
        ...
        let Some(value) = module.evaluate(tc_scope) else {
            let exception = tc_scope.exception().unwrap();
            return Err(
                CoreErrorKind::Js(JsError::from_v8_exception(tc_scope, exception)).into_box(),
            );
        };
        ...
        match promise.state() {
            ...
            PromiseState::Rejected => {
                ...
                Err(CoreErrorKind::Js(JsError::from_v8_exception(tc_scope, err)).into_box())
            }
            PromiseState::Pending => {
                unreachable!()
            }
    ...
    fn drain_prepare_dyn_imports(&self, cx: &mut Context, scope: &mut v8::PinScope) {
    ...
    fn drain_code_cache_ready(&self, cx: &mut Context) {
        ...
        while let Poll::Ready(Some(_)) = self.code_cache_ready_futs.poll_next_unpin(cx) {}
    }
    ...
    pub(crate) fn lazy_load_es_module_with_code(
        ...
        let previous_loading_internal_modules = is_internal_scheme(specifier.scheme())
    ...
    fn lazy_load_es_module_with_code_inner(
        ...
        if !result.is_undefined() {
            return Err(CoreErrorKind::Js(exception_to_err(scope, result, false, true)).into_box());
        }
    ...
    pub(crate) fn has_lazy_esm_source(&self, specifier: &str) -> bool {
        ...
        data.known_lazy_esm.borrow().contains(specifier)
            || residual_static_table_contains(data.residual_lazy_esm_sources, specifier)
    }
    ...
    /// time.
    pub(crate) fn take_lazy_esm_source(&self, specifier: &str) -> Option<ModuleCodeString> {
        let data = self.data.borrow();
        ...
        let entry = match sources.get_mut(specifier) {
            ...
            None => {
                let source =
                    residual_source_from_static_table(data.residual_lazy_esm_sources, specifier)?;
                data.consumed_lazy_specifiers
                    .borrow_mut()
    ...
    pub(crate) fn lazy_load_synthetic_esm_module(
        ...
        let cached_handle = {
            let data = self.data.borrow();
            data.get_id(module_specifier, RequestedModuleType::None)
        ...
        if let Some(handle) = cached_handle {
            ...
            if handle_local.get_status() == v8::ModuleStatus::Instantiated {
                ...
                if !result.is_undefined() {
                    return Err(
                        CoreErrorKind::Js(exception_to_err(scope, result, false, true)).into_box(),
                    );
                }
            }
    ...
    pub(crate) fn lazy_load_esm_module(
        ...
        let loader = LazyEsmModuleLoader::new(lazy_esm_sources, residual_lazy_esm_sources);
        ...
        if let Some((cached_id, handle)) = cached_id_and_handle {
            crate::modules::import_graph::record_lazy_esm_cached(scope, module_specifier);
            let handle_local = v8::Local::new(scope, handle);
        ...
        self.lazy_load_es_module_with_code(scope, module_specifier, source_code, code_cache_info)
    }
    ...
    pub(crate) fn build_synthetic_esm_module(
        ...
        let exports_obj = v8::Local::<v8::Object>::try_from(exports_local).map_err(|_| {
    ...
    pub(crate) fn load_ext_script(
        ...
        // Cache hit: return the previously evaluated result.
        {
            ...
            if let Some(cached) = data.loaded_script_results.borrow().get(&specifier_key) {
        ...
        let source = {
            ...
            sources.remove(&specifier_key).or_else(|| {
                residual_source_from_static_table(data.residual_lazy_script_sources, specifier)
            })
        };
        ...
        let v8_source = if AsRef::<str>::as_ref(&source).starts_with("\"use strict\"; return (") {
        ...
        if (!had_data || rejected)
            && let (Some(spec), Some(info)) = (cache_specifier.as_ref(), code_cache_info.as_ref())
            && let Some(cache) = function.create_code_cache()
        {
            let fut = self
                .loader
                .borrow()
                .code_cache_ready(spec.clone(), info.hash, &cache[..]);
            self.code_cache_ready_futs.push(fut);
        }
...
pub fn wrap_lazy_ext_script(source: &str) -> String {
    ...
    let trimmed = expr_start.trim_end_matches(|c: char| c.is_whitespace() || c == ';');
...
const WASM_GLOBAL_UNWRAP_HELPER: &str =
...
fn render_js_wasm_module(specifier: &str, wasm_deps: WasmDeps) -> String {
    ...
    StringBuilder::build(|builder| {
        ...
        if has_global_export {
            ...
            builder.append("import.meta.wasmInstances.set(selfNs, modInstance.exports);\n");
    ...
    let deps = WasmDeps {
        imports: vec![
            ...
            wasm_dep_analyzer::Import {
                name: "counter",
                module: "./dep.wasm",
                import_type: wasm_dep_analyzer::ImportType::Global(wasm_dep_analyzer::GlobalType {
