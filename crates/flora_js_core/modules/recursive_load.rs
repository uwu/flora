...
impl RecursiveModuleLoad {
    ...
    pub(crate) fn new_dynamic_import(
        ...
        Self::new_with_resolved_specifier(
            LoadInit::DynamicImport(specifier, referrer, requested_module_type, phase),
            module_map_rc,
            resolved_specifier,
        )
    }
    ...
    fn new_with_resolved_specifier(
        ...
        let root_module_id = resolved_specifier
            ...
            .and_then(|spec| {
                module_map_rc.get_id(spec.as_str(), Self::requested_module_type_from_init(&init))
            });
        Self {
            id,
            root_module_id,
    ...
    fn resolve_root_from_init(
        ...
        match init {
            ...
            LoadInit::DynamicImport(specifier, referrer, _, _) => {
                module_map_rc.resolve(specifier, referrer, ResolutionKind::DynamicImport)
            }
        }
    }
    ...
    fn register_and_recurse_impl(
            ...
            .has_synthetic_esm_module(module_request.reference.specifier.as_str())
        {
            let module_id = self
                .module_map_rc
                .build_synthetic_esm_module(scope, module_request.reference.specifier.as_str())
                .map_err(|e| {
        ...
        if let Some(source_kind) = ModuleSourceKind::from_module_type(&module_source.module_type) {
            match source_kind {
    ...
    fn register_and_recurse_inner(
        ...
        while let Some((module_id, module_reference)) = already_registered.pop_front() {
    ...
    pub(crate) async fn run_to_completion(
        mut self,
        mut step: impl FnMut(&mut Self, RegisterStep<'_>) -> Result<RegisterOutcome, CoreError>,
...
impl RecursiveModuleLoad {
    ...
    fn init_with_resolved_root(
        ...
        let (requested_module_type, phase) = match &inner.init {
            LoadInit::DynamicImport(_, _, module_type, phase) => (module_type.clone(), *phase),
            ...
            && let Some(module_id) = inner.root_module_id
        {
            ...
            // loads).
            inner.register_and_recurse_inner(module_id, &module_request.reference, None);
...
fn source_mapped_module_load_referrer(
    ...
    let (specifier, line_number, column_number) = match source_mapper.borrow_mut().apply_source_map(
        referrer.as_str(),
        line_number,
        column_number,
    ) {
        SourceMapApplication::Unchanged => (referrer.clone(), line_number as _, column_number as _),
