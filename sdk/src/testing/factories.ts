...
function deepMerge<T>(defaults: T, partial?: DeepPartial<T>): T {
  ...
  for (const key of Object.keys(partial as object)) {
    ...
    if (
      pVal !== undefined && typeof pVal === 'object' && pVal !== null && !Array.isArray(pVal) &&
...
export function makeUser(partial?: DeepPartial<EventUser>): EventUser {
  return deepMerge<EventUser>(
    { id: nextId(), username: 'testuser', bot: false },
    partial
  )
}
...
export function makeInteraction(
  ...
  const discordOptions = opts
    ? Object.entries(opts).map(([k, v]) => optionToDiscord(k, v))
    : []
