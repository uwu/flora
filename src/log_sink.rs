...
impl LogBuffer {
    fn new(capacity: usize) -> Self {
        Self { entries: Vec::with_capacity(capacity), index: 0 }
    }
    ...
    fn filter_by_guild(&self, guild_id: &str, count: usize) -> Vec<LogEntry> {
        ...
        let iter: Box<dyn Iterator<Item = &LogEntry>> = if len < self.entries.capacity() {
            ...
            let newest = if self.index == 0 { len - 1 } else { self.index - 1 };
            Box::new(
                self.entries[..=newest].iter().rev().chain(self.entries[newest + 1..].iter().rev()),
            )
        };
...
impl LogSink {
    ...
    fn new() -> Self {
        ...
        Self { sender, buffer: Arc::new(RwLock::new(LogBuffer::new(LOG_BUFFER_SIZE))) }
    }
...
mod tests {
    ...
    fn test_log_buffer_filter_by_guild() {
        ...
        assert!(guild_a.iter().all(|e| e.guild_id.as_deref() == Some("guild_a")));
    }
}
